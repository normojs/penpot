...
function assertP25124RevocationAppealResolutionEnforcementEvidenceAttestationNotarizationCertificationEndorsementCountersignatureVerificationRevocationAppealResolutionEnforcementEvidenceAttestationPolicyMetadataOnly(policy) {
    ...
    assert.ok(policy[auditAttestationTopic + "Checks"].some((check) => check.id === "audit-access-granted" && check.executed === false));
    assert.ok(policy[auditAttestationTopic + "Checks"].some((check) => check.id.endsWith("-record-present") && check.executed === false));
    assert.ok(policy[auditAttestationTopic + "Checks"].some((check) => check.id.endsWith("-not-created") && check.executed === false));
    assert.ok(policy[auditAttestationTopic + "Checks"].some((check) => check.id.endsWith("-record-not-stored") && check.executed === false));
    ...
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not prepare, create, validate, store, or publish attestations")));
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not read, attest, or verify countersignature verification revocation appeal resolution enforcement evidence records or bundles")));
...
function assertP25125RevocationAppealResolutionEnforcementEvidenceAttestationNotarizationPolicyMetadataOnly(policy) {
    ...
    assert.ok(policy[auditNotarizationTopic + "Checks"].some((check) => check.id === "audit-access-granted" && check.executed === false));
    assert.ok(policy[auditNotarizationTopic + "Checks"].some((check) => check.id.endsWith("-record-present") && check.executed === false));
    assert.ok(policy[auditNotarizationTopic + "Checks"].some((check) => check.id.endsWith("-not-created") && check.executed === false));
    assert.ok(policy[auditNotarizationTopic + "Checks"].some((check) => check.id.endsWith("-record-not-stored") && check.executed === false));
    ...
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not prepare, create, validate, store, or publish notarizations")));
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not read, notarize, or verify countersignature verification revocation appeal resolution enforcement evidence attestations")));
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not register runtime dispatch")));
    assert.ok(policy.requiredBeforeRuntimeDispatch.includes("keep render.thumbnail unavailable until executable adapter registration is approved"));
}

function assertP25126RevocationAppealResolutionEnforcementEvidenceAttestationNotarizationCertificationPolicyMetadataOnly(policy) {
    ...
    assert.ok(policy[auditCertificationTopic + "Checks"].some((check) => check.id === "audit-access-granted" && check.executed === false));
    assert.ok(policy[auditCertificationTopic + "Checks"].some((check) => check.id.endsWith("-record-present") && check.executed === false));
    assert.ok(policy[auditCertificationTopic + "Checks"].some((check) => check.id.endsWith("-policy-selected") && check.executed === false));
    assert.ok(policy[auditCertificationTopic + "Checks"].some((check) => check.id.endsWith("-not-created") && check.executed === false));
    assert.ok(policy[auditCertificationTopic + "Checks"].some((check) => check.id.endsWith("-record-not-stored") && check.executed === false));
    ...
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not prepare, create, validate, store, or publish certifications")));
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not read, certify, or verify countersignature verification revocation appeal resolution enforcement evidence attestation notarizations")));
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not register runtime dispatch")));
    assert.ok(policy.requiredBeforeRuntimeDispatch.includes("keep render.thumbnail unavailable until executable adapter registration is approved"));
}

function assertP25127RevocationAppealResolutionEnforcementEvidenceAttestationNotarizationCertificationEndorsementPolicyMetadataOnly(policy) {
    ...
    assert.ok(policy[auditEndorsementTopic + "Checks"].some((check) => check.id === "audit-access-granted" && check.executed === false));
    assert.ok(policy[auditEndorsementTopic + "Checks"].some((check) => check.id.endsWith("-not-created") && check.executed === false));
    ...
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not prepare, create, validate, store, or publish endorsements")));
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not read, endorse, or verify certifications")));
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not register runtime dispatch")));
    assert.ok(policy.requiredBeforeRuntimeDispatch.includes("keep render.thumbnail unavailable until executable adapter registration is approved"));
}

function assertP25128RevocationAppealResolutionEnforcementEvidenceAttestationNotarizationCertificationEndorsementCountersignaturePolicyMetadataOnly(policy) {
    const capitalEndorsementTopic = P25127_POLICY_KEY.replace(/^packageMaterializationApprovalAudit/, "").replace(/Policy$/, "");
    ...
    assert.ok(policy, "P25.128 policy payload");
    assert.equal(policy.status, "planned-disabled");
    assert.equal(policy[auditCountersignatureTopic + "Version"], "P25.128");
    assert.equal(policy.adapter, "renderer-service");
    assert.equal(policy.command, "render.thumbnail");
    assert.equal(policy.dryRunOnly, true);
    assert.equal(policy.approvalRequired, true);
    ...
    assert.equal(policy.consumes[P25127_POLICY_KEY].currentStatus, "planned-disabled");
    assert.equal(policy.consumes[P25127_POLICY_KEY][auditEndorsementTopic + "Version"], "P25.127");
    assert.equal(policy.consumes[P25127_POLICY_KEY][endorsementTopic + "Created"], false);
    assert.equal(policy.consumes[P25127_POLICY_KEY][endorsementTopic + "Stored"], false);
    assert.equal(policy.consumes[P25127_POLICY_KEY][endorsementTopic + "RecordRead"], false);
    assert.equal(policy.consumes[P25127_POLICY_KEY][endorsementTopic + "Countersigned"], false);
    assert.equal(policy.consumes.packageMaterializationApprovalAuditAccessPolicy.auditAccessVersion, "P25.53");
    assert.equal(policy.consumes.packageMaterializationApprovalAuditAccessPolicy.auditRecordRead, false);
    assert.equal(policy.consumes.packageMaterializationApprovalAuditAccessPolicy.accessGranted, false);
    assert.equal(policy.consumes.packageMaterializationFinalApprovalChecklist.checklistVersion, "P25.40");
    ...
    assert.ok(policy[auditCountersignatureTopic + "Checks"].some((check) => check.id.endsWith("-not-created") && check.executed === false));
    assert.ok(policy[auditCountersignatureTopic + "Checks"].some((check) => check.id.endsWith("-record-not-stored") && check.executed === false));
    ...
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not read, countersign, or verify endorsements")));
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not register runtime dispatch")));
    assert.ok(policy.requiredBeforeRuntimeDispatch.includes("keep render.thumbnail unavailable until executable adapter registration is approved"));
}
...
const P25132_POLICY_KEY = P25131_POLICY_KEY.replace(/AppealPolicy$/, "AppealResolutionPolicy");
const P25133_POLICY_KEY = P25132_POLICY_KEY.replace(/ResolutionPolicy$/, "ResolutionEnforcementPolicy");
const P25134_POLICY_KEY = P25133_POLICY_KEY.replace(/EnforcementPolicy$/, "EnforcementEvidencePolicy");
...
const P25140_POLICY_KEY = P25139_POLICY_KEY.replace(/CountersignaturePolicy$/, "CountersignatureVerificationPolicy");
const P25141_POLICY_KEY = P25140_POLICY_KEY.replace(/VerificationPolicy$/, "VerificationRevocationPolicy");
const P25142_POLICY_KEY = P25141_POLICY_KEY.replace(/RevocationPolicy$/, "RevocationAppealPolicy");
const P25143_POLICY_KEY = P25142_POLICY_KEY.replace(/AppealPolicy$/, "AppealResolutionPolicy");
...
function assertP25106CountersignaturePolicyMetadataOnly(policy) {
...
function assertP25107VerificationPolicyMetadataOnly(policy) {
    ...
    assert.ok(policy[`${auditVerificationTopic}Checks`].some((check) => check.id === "audit-access-granted" && check.executed === false));
    ...
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not read or verify signatures")));
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not prepare, execute, store, or publish verification results")));
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not register runtime dispatch")));
    assert.ok(policy.requiredBeforeRuntimeDispatch.includes("keep render.thumbnail unavailable until executable adapter registration is approved"));
}


function assertP25108RevocationPolicyMetadataOnly(policy) {
    ...
    assert.ok(policy[`${auditRevocationTopic}Checks`].some((check) => check.id === "audit-access-granted" && check.executed === false));
    ...
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not revoke verification results")));
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not read countersignature verification or verification records")));
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not register runtime dispatch")));
    assert.ok(policy.requiredBeforeRuntimeDispatch.includes("keep render.thumbnail unavailable until executable adapter registration is approved"));
}


function assertP25109RevocationAppealPolicyMetadataOnly(policy) {
...
function assertP25111RevocationAppealResolutionEnforcementPolicyMetadataOnly(policy) {
    ...
    assert.ok(policy[`${auditEnforcementTopic}Checks`].some((check) => check.id === "audit-access-granted" && check.executed === false));
    ...
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not enforce countersignature verification revocation appeal resolutions")));
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not accept or reject countersignature verification revocation appeal resolution enforcement decisions")));
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not register runtime dispatch")));
    assert.ok(policy.requiredBeforeRuntimeDispatch.includes("keep render.thumbnail unavailable until executable adapter registration is approved"));
}

function assertP25112RevocationAppealResolutionEnforcementEvidencePolicyMetadataOnly(policy) {
    ...
    assert.ok(policy[auditEvidenceTopic + "Checks"].some((check) => check.id === "audit-access-granted" && check.executed === false));
    ...
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not collect, validate, or normalize evidence")));
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not read appeal resolution enforcement records")));
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not register runtime dispatch")));
    assert.ok(policy.requiredBeforeRuntimeDispatch.includes("keep render.thumbnail unavailable until executable adapter registration is approved"));
}

function assertP25113RevocationAppealResolutionEnforcementEvidenceAttestationPolicyMetadataOnly(policy) {
    ...
    assert.equal(policy[auditAttestationTopic + "Decision"].canEnableRuntimeDispatch, false);
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not prepare, create, validate, store, or publish attestations")));
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not read, attest, or verify evidence records")));
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not register runtime dispatch")));
    assert.ok(policy.requiredBeforeRuntimeDispatch.includes("keep render.thumbnail unavailable until executable adapter registration is approved"));
}

function assertP25114RevocationAppealResolutionEnforcementEvidenceAttestationNotarizationPolicyMetadataOnly(policy) {
    ...
    assert.equal(policy[auditNotarizationTopic + "Decision"].canEnableRuntimeDispatch, false);
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not prepare, create, validate, store, or publish notarizations")));
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not read, notarize, or verify attestations")));
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not register runtime dispatch")));
    assert.ok(policy.requiredBeforeRuntimeDispatch.includes("keep render.thumbnail unavailable until executable adapter registration is approved"));
}

function assertP25115RevocationAppealResolutionEnforcementEvidenceAttestationNotarizationCertificationPolicyMetadataOnly(policy) {
    ...
    assert.equal(policy[auditCertificationTopic + "Decision"].canEnableRuntimeDispatch, false);
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not prepare, create, validate, store, or publish certifications")));
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not read, certify, or verify notarizations")));
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not register runtime dispatch")));
    assert.ok(policy.requiredBeforeRuntimeDispatch.includes("keep render.thumbnail unavailable until executable adapter registration is approved"));
}

function assertP25116RevocationAppealResolutionEnforcementEvidenceAttestationNotarizationCertificationEndorsementPolicyMetadataOnly(policy) {
...
function assertP25117RevocationAppealResolutionEnforcementEvidenceAttestationNotarizationCertificationEndorsementCountersignaturePolicyMetadataOnly(policy) {
...
function assertP25118RevocationAppealResolutionEnforcementEvidenceAttestationNotarizationCertificationEndorsementCountersignatureVerificationPolicyMetadataOnly(policy) {
    ...
    assert.ok(policy[auditVerificationTopic + "Checks"].some((check) => check.id === "audit-access-granted" && check.executed === false));
    assert.ok(policy[auditVerificationTopic + "Checks"].some((check) => check.id.endsWith("-not-executed") && check.executed === false));
    assert.ok(policy[auditVerificationTopic + "Checks"].some((check) => check.id.endsWith("-result-not-stored") && check.executed === false));
    assert.equal(policy[auditVerificationTopic + "Decision"].status, "blocked");
    ...
    assert.equal(policy[auditVerificationTopic + "Decision"].canEnableRuntimeDispatch, false);
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not read countersignatures or countersignature records")));
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not prepare, execute, store, or publish countersignature verification results")));
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not register runtime dispatch")));
    assert.ok(policy.requiredBeforeRuntimeDispatch.includes("keep render.thumbnail unavailable until executable adapter registration is approved"));
}

function assertP25119RevocationAppealResolutionEnforcementEvidenceAttestationNotarizationCertificationEndorsementCountersignatureVerificationRevocationPolicyMetadataOnly(policy) {
    ...
    assert.ok(policy[auditRevocationTopic + "Checks"].some((check) => check.id === "audit-access-granted" && check.executed === false));
    assert.ok(policy[auditRevocationTopic + "Checks"].some((check) => check.id.endsWith("-not-revoked") && check.executed === false));
    assert.ok(policy[auditRevocationTopic + "Checks"].some((check) => check.id.endsWith("-record-not-created") && check.executed === false));
    assert.ok(policy[auditRevocationTopic + "Checks"].some((check) => check.id.endsWith("-record-not-stored") && check.executed === false));
    assert.equal(policy[auditRevocationTopic + "Decision"].status, "blocked");
    ...
    assert.equal(policy[auditRevocationTopic + "Decision"].canEnableRuntimeDispatch, false);
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not capture revocation reasons or compute revocation scopes")));
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not revoke countersignature verification results")));
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not read countersignature verification results or verification records")));
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not register runtime dispatch")));
    assert.ok(policy.requiredBeforeRuntimeDispatch.includes("keep render.thumbnail unavailable until executable adapter registration is approved"));
}


function assertP25120RevocationAppealResolutionEnforcementEvidenceAttestationNotarizationCertificationEndorsementCountersignatureVerificationRevocationAppealPolicyMetadataOnly(policy) {
...
function assertP25121RevocationAppealResolutionEnforcementEvidenceAttestationNotarizationCertificationEndorsementCountersignatureVerificationRevocationAppealResolutionPolicyMetadataOnly(policy) {
    ...
    assert.ok(policy[auditResolutionTopic + "Checks"].some((check) => check.id === "audit-access-granted" && check.executed === false));
    assert.ok(policy[auditResolutionTopic + "Checks"].some((check) => check.id.endsWith("-record-present") && check.executed === false));
    assert.ok(policy[auditResolutionTopic + "Checks"].some((check) => check.id.endsWith("-not-resolved") && check.executed === false));
    assert.ok(policy[auditResolutionTopic + "Checks"].some((check) => check.id.endsWith("-record-not-stored") && check.executed === false));
    assert.equal(policy[auditResolutionTopic + "Decision"].status, "blocked");
    ...
    assert.equal(policy[auditResolutionTopic + "Decision"].canEnableRuntimeDispatch, false);
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not resolve countersignature verification revocation appeals")));
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not accept, reject, enforce, evidence, attest, notarize, certify, endorse, countersign, verify, or revoke appeal resolutions")));
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not read countersignature verification revocation, verification, countersignature, or audit records")));
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not register runtime dispatch")));
    assert.ok(policy.requiredBeforeRuntimeDispatch.includes("keep render.thumbnail unavailable until executable adapter registration is approved"));
}

function assertP25122RevocationAppealResolutionEnforcementEvidenceAttestationNotarizationCertificationEndorsementCountersignatureVerificationRevocationAppealResolutionEnforcementPolicyMetadataOnly(policy) {
    ...
    assert.ok(policy[auditEnforcementTopic + "Checks"].some((check) => check.id === "audit-access-granted" && check.executed === false));
    assert.ok(policy[auditEnforcementTopic + "Checks"].some((check) => check.id.endsWith("-record-present") && check.executed === false));
    assert.ok(policy[auditEnforcementTopic + "Checks"].some((check) => check.id.endsWith("-not-enforced") && check.executed === false));
    assert.ok(policy[auditEnforcementTopic + "Checks"].some((check) => check.id.endsWith("-record-not-stored") && check.executed === false));
    assert.equal(policy[auditEnforcementTopic + "Decision"].status, "blocked");
    ...
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not enforce countersignature verification revocation appeal resolutions")));
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not accept or reject countersignature verification revocation appeal resolution enforcement decisions")));
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not read countersignature verification revocation appeal resolutions, resolution records, appeals, appeal records, verification records, revocation records, countersignatures, or audit records")));
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not register runtime dispatch")));
    assert.ok(policy.requiredBeforeRuntimeDispatch.includes("keep render.thumbnail unavailable until executable adapter registration is approved"));
}
...
function assertP25123RevocationAppealResolutionEnforcementEvidenceAttestationNotarizationCertificationEndorsementCountersignatureVerificationRevocationAppealResolutionEnforcementEvidencePolicyMetadataOnly(policy) {
    ...
    assert.ok(policy[auditEvidenceTopic + "Checks"].some((check) => check.id === "audit-access-granted" && check.executed === false));
    assert.ok(policy[auditEvidenceTopic + "Checks"].some((check) => check.id.endsWith("-record-present") && check.executed === false));
    assert.ok(policy[auditEvidenceTopic + "Checks"].some((check) => check.id.endsWith("-not-collected") && check.executed === false));
    assert.ok(policy[auditEvidenceTopic + "Checks"].some((check) => check.id.endsWith("-record-not-created") && check.executed === false));
    assert.equal(policy[auditEvidenceTopic + "Decision"].status, "blocked");
    ...
    assert.equal(policy[auditEvidenceTopic + "Decision"].canEnableRuntimeDispatch, false);
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not collect, validate, or normalize countersignature verification revocation appeal resolution enforcement evidence")));
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not read countersignature verification revocation appeal resolution enforcement")));
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not register runtime dispatch")));
    assert.ok(policy.requiredBeforeRuntimeDispatch.includes("keep render.thumbnail unavailable until executable adapter registration is approved"));
}

function assertP25129RevocationAppealResolutionEnforcementEvidenceAttestationNotarizationCertificationEndorsementCountersignatureVerificationPolicyMetadataOnly(policy) {
    const capitalCountersignatureTopic = P25128_POLICY_KEY.replace(/^packageMaterializationApprovalAudit/, "").replace(/Policy$/, "");
    const countersignatureTopic = capitalCountersignatureTopic[0].toLowerCase() + capitalCountersignatureTopic.slice(1);
    const capitalVerificationTopic = capitalCountersignatureTopic + "Verification";
    const verificationTopic = countersignatureTopic + "Verification";
    const auditCountersignatureTopic = "audit" + capitalCountersignatureTopic;
    ...
    assert.ok(policy, "P25.129 policy payload");
    assert.equal(policy.status, "planned-disabled");
    assert.equal(policy[auditVerificationTopic + "Version"], "P25.129");
    assert.equal(policy.adapter, "renderer-service");
    assert.equal(policy.command, "render.thumbnail");
    assert.equal(policy.dryRunOnly, true);
    assert.equal(policy.approvalRequired, true);
    ...
    assert.equal(policy.consumes[P25128_POLICY_KEY].currentStatus, "planned-disabled");
    assert.equal(policy.consumes[P25128_POLICY_KEY][auditCountersignatureTopic + "Version"], "P25.128");
    assert.equal(policy.consumes[P25128_POLICY_KEY][countersignatureTopic + "Created"], false);
    assert.equal(policy.consumes[P25128_POLICY_KEY][countersignatureTopic + "Stored"], false);
    assert.equal(policy.consumes[P25128_POLICY_KEY][countersignatureTopic + "RecordStored"], false);
    assert.equal(policy.consumes[P25128_POLICY_KEY][countersignatureTopic + "RecordRead"], false);
    assert.equal(policy.consumes[P25128_POLICY_KEY][countersignatureTopic + "Verified"], false);
    assert.equal(policy.consumes.packageMaterializationApprovalAuditAccessPolicy.auditAccessVersion, "P25.53");
    assert.equal(policy.consumes.packageMaterializationApprovalAuditAccessPolicy.auditRecordRead, false);
    assert.equal(policy.consumes.packageMaterializationApprovalAuditAccessPolicy.accessGranted, false);
    assert.equal(policy.consumes.packageMaterializationFinalApprovalChecklist.checklistVersion, "P25.40");
...
function assertP25130RevocationAppealResolutionEnforcementEvidenceAttestationNotarizationCertificationEndorsementCountersignatureVerificationRevocationPolicyMetadataOnly(policy) {
    const capitalVerificationTopic = P25129_POLICY_KEY.replace(/^packageMaterializationApprovalAudit/, "").replace(/Policy$/, "");
    const verificationTopic = capitalVerificationTopic[0].toLowerCase() + capitalVerificationTopic.slice(1);
    const capitalRevocationTopic = capitalVerificationTopic + "Revocation";
    const revocationTopic = verificationTopic + "Revocation";
    const capitalCountersignatureTopic = capitalVerificationTopic.replace(/Verification$/, "");
    ...
    assert.ok(policy, "P25.130 policy payload");
    assert.equal(policy.status, "planned-disabled");
    assert.equal(policy[auditRevocationTopic + "Version"], "P25.130");
    assert.equal(policy.adapter, "renderer-service");
    assert.equal(policy.command, "render.thumbnail");
    assert.equal(policy.dryRunOnly, true);
    assert.equal(policy.approvalRequired, true);
    ...
    assert.equal(policy.consumes[P25129_POLICY_KEY].currentStatus, "planned-disabled");
    assert.equal(policy.consumes[P25129_POLICY_KEY][auditVerificationTopic + "Version"], "P25.129");
    assert.equal(policy.consumes[P25129_POLICY_KEY][verificationTopic + "Executed"], false);
    assert.equal(policy.consumes[P25129_POLICY_KEY][verificationTopic + "Stored"], false);
    assert.equal(policy.consumes[P25129_POLICY_KEY][verificationTopic + "RecordStored"], false);
    assert.equal(policy.consumes[P25129_POLICY_KEY][verificationTopic + "RecordRead"], false);
    assert.equal(policy.consumes[P25129_POLICY_KEY][verificationTopic + "Passed"], false);
    assert.equal(policy.consumes[P25129_POLICY_KEY][verificationTopic + "Verified"], false);
    assert.equal(policy.consumes[P25129_POLICY_KEY][verificationTopic + "Revoked"], false);
    assert.equal(policy.consumes.packageMaterializationApprovalAuditAccessPolicy.auditAccessVersion, "P25.53");
    assert.equal(policy.consumes.packageMaterializationApprovalAuditAccessPolicy.auditRecordRead, false);
    assert.equal(policy.consumes.packageMaterializationApprovalAuditAccessPolicy.accessGranted, false);
    assert.equal(policy.consumes.packageMaterializationFinalApprovalChecklist.checklistVersion, "P25.40");
...
function assertP25132RevocationAppealResolutionEnforcementEvidenceAttestationNotarizationCertificationEndorsementCountersignatureVerificationRevocationAppealResolutionPolicyMetadataOnly(policy) {
    const capitalAppealTopic = P25131_POLICY_KEY.replace(/^packageMaterializationApprovalAudit/, "").replace(/Policy$/, "");
    const appealTopic = capitalAppealTopic[0].toLowerCase() + capitalAppealTopic.slice(1);
    const capitalRevocationTopic = capitalAppealTopic.replace(/Appeal$/, "");
    const revocationTopic = appealTopic.replace(/Appeal$/, "");
    const capitalResolutionTopic = capitalAppealTopic + "Resolution";
    ...
    assert.ok(policy, "P25.132 policy payload");
    assert.equal(policy.status, "planned-disabled");
    assert.equal(policy[auditResolutionTopic + "Version"], "P25.132");
    assert.equal(policy.adapter, "renderer-service");
    assert.equal(policy.command, "render.thumbnail");
    assert.equal(policy.dryRunOnly, true);
    assert.equal(policy.approvalRequired, true);
    ...
    assert.equal(policy.consumes[P25131_POLICY_KEY].currentStatus, "planned-disabled");
    assert.equal(policy.consumes[P25131_POLICY_KEY][auditAppealTopic + "Version"], "P25.131");
    assert.equal(policy.consumes[P25131_POLICY_KEY][revocationTopic + "Appealed"], false);
    assert.equal(policy.consumes[P25131_POLICY_KEY][appealTopic + "RecordStored"], false);
    assert.equal(policy.consumes[P25131_POLICY_KEY][appealTopic + "RecordRead"], false);
    assert.equal(policy.consumes[P25131_POLICY_KEY][appealTopic + "Resolved"], false);
    assert.equal(policy.consumes.packageMaterializationApprovalAuditAccessPolicy.auditAccessVersion, "P25.53");
    assert.equal(policy.consumes.packageMaterializationApprovalAuditAccessPolicy.auditRecordRead, false);
    assert.equal(policy.consumes.packageMaterializationApprovalAuditAccessPolicy.accessGranted, false);
    assert.equal(policy.consumes.packageMaterializationFinalApprovalChecklist.checklistVersion, "P25.40");
...
function assertP25133RevocationAppealResolutionEnforcementEvidenceAttestationNotarizationCertificationEndorsementCountersignatureVerificationRevocationAppealResolutionEnforcementPolicyMetadataOnly(policy) {
    const capitalResolutionTopic = P25132_POLICY_KEY.replace(/^packageMaterializationApprovalAudit/, "").replace(/Policy$/, "");
    const resolutionTopic = capitalResolutionTopic[0].toLowerCase() + capitalResolutionTopic.slice(1);
    const capitalAppealTopic = capitalResolutionTopic.replace(/Resolution$/, "");
    const appealTopic = resolutionTopic.replace(/Resolution$/, "");
    const capitalEnforcementTopic = capitalResolutionTopic + "Enforcement";
    ...
    assert.ok(policy, "P25.133 policy payload");
    assert.equal(policy.status, "planned-disabled");
    assert.equal(policy[auditEnforcementTopic + "Version"], "P25.133");
    assert.equal(policy.adapter, "renderer-service");
    assert.equal(policy.command, "render.thumbnail");
    assert.equal(policy.dryRunOnly, true);
    assert.equal(policy.approvalRequired, true);
    ...
    assert.equal(policy.consumes[P25132_POLICY_KEY].currentStatus, "planned-disabled");
    assert.equal(policy.consumes[P25132_POLICY_KEY][auditResolutionTopic + "Version"], "P25.132");
    assert.equal(policy.consumes[P25132_POLICY_KEY][appealTopic + "Resolved"], false);
    assert.equal(policy.consumes[P25132_POLICY_KEY][resolutionTopic + "RecordStored"], false);
    assert.equal(policy.consumes[P25132_POLICY_KEY][resolutionTopic + "RecordRead"], false);
    assert.equal(policy.consumes[P25132_POLICY_KEY][resolutionTopic + "Enforced"], false);
    assert.equal(policy.consumes.packageMaterializationApprovalAuditAccessPolicy.auditAccessVersion, "P25.53");
    assert.equal(policy.consumes.packageMaterializationApprovalAuditAccessPolicy.auditRecordRead, false);
    assert.equal(policy.consumes.packageMaterializationApprovalAuditAccessPolicy.accessGranted, false);
    assert.equal(policy.consumes.packageMaterializationFinalApprovalChecklist.checklistVersion, "P25.40");
...
function assertP25134RevocationAppealResolutionEnforcementEvidenceAttestationNotarizationCertificationEndorsementCountersignatureVerificationRevocationAppealResolutionEnforcementEvidencePolicyMetadataOnly(policy) {
    const capitalEnforcementTopic = P25133_POLICY_KEY.replace(/^packageMaterializationApprovalAudit/, "").replace(/Policy$/, "");
    const enforcementTopic = capitalEnforcementTopic[0].toLowerCase() + capitalEnforcementTopic.slice(1);
    const capitalResolutionTopic = capitalEnforcementTopic.replace(/Enforcement$/, "");
    const resolutionTopic = enforcementTopic.replace(/Enforcement$/, "");
    const capitalEvidenceTopic = capitalEnforcementTopic + "Evidence";
    ...
    assert.ok(policy, "P25.134 policy payload");
    assert.equal(policy.status, "planned-disabled");
    assert.equal(policy[auditEvidenceTopic + "Version"], "P25.134");
    assert.equal(policy.adapter, "renderer-service");
    assert.equal(policy.command, "render.thumbnail");
    assert.equal(policy.dryRunOnly, true);
    assert.equal(policy.approvalRequired, true);
    ...
    assert.equal(policy.consumes[P25133_POLICY_KEY].currentStatus, "planned-disabled");
    assert.equal(policy.consumes[P25133_POLICY_KEY][auditEnforcementTopic + "Version"], "P25.133");
    assert.equal(policy.consumes[P25133_POLICY_KEY][resolutionTopic + "Enforced"], false);
    assert.equal(policy.consumes[P25133_POLICY_KEY][enforcementTopic + "RecordStored"], false);
    assert.equal(policy.consumes[P25133_POLICY_KEY][enforcementTopic + "RecordRead"], false);
    assert.equal(policy.consumes[P25133_POLICY_KEY][evidenceTopic + "Collected"], false);
    assert.equal(policy.consumes.packageMaterializationApprovalAuditAccessPolicy.auditAccessVersion, "P25.53");
    assert.equal(policy.consumes.packageMaterializationApprovalAuditAccessPolicy.auditRecordRead, false);
    assert.equal(policy.consumes.packageMaterializationApprovalAuditAccessPolicy.accessGranted, false);
    assert.equal(policy.consumes.packageMaterializationFinalApprovalChecklist.checklistVersion, "P25.40");
...
function assertP25135RevocationAppealResolutionEnforcementEvidenceAttestationNotarizationCertificationEndorsementCountersignatureVerificationRevocationAppealResolutionEnforcementEvidenceAttestationPolicyMetadataOnly(policy) {
    const capitalEvidenceTopic = P25134_POLICY_KEY.replace(/^packageMaterializationApprovalAudit/, "").replace(/Policy$/, "");
    const evidenceTopic = capitalEvidenceTopic[0].toLowerCase() + capitalEvidenceTopic.slice(1);
    const capitalAttestationTopic = capitalEvidenceTopic + "Attestation";
    const attestationTopic = evidenceTopic + "Attestation";
    const auditEvidenceTopic = "audit" + capitalEvidenceTopic;
    ...
    assert.ok(policy, "P25.135 policy payload");
    assert.equal(policy.status, "planned-disabled");
    assert.equal(policy[auditAttestationTopic + "Version"], "P25.135");
    assert.equal(policy.adapter, "renderer-service");
    assert.equal(policy.command, "render.thumbnail");
    assert.equal(policy.dryRunOnly, true);
    assert.equal(policy.approvalRequired, true);
    ...
    assert.equal(policy.consumes[P25134_POLICY_KEY].currentStatus, "planned-disabled");
    assert.equal(policy.consumes[P25134_POLICY_KEY][auditEvidenceTopic + "Version"], "P25.134");
    assert.equal(policy.consumes[P25134_POLICY_KEY][evidenceTopic + "RecordCreated"], false);
    assert.equal(policy.consumes[P25134_POLICY_KEY][evidenceTopic + "RecordStored"], false);
    assert.equal(policy.consumes[P25134_POLICY_KEY][evidenceTopic + "BundleStored"], false);
    assert.equal(policy.consumes[P25134_POLICY_KEY][evidenceTopic + "Attested"], false);
    assert.equal(policy.consumes.packageMaterializationApprovalAuditAccessPolicy.auditAccessVersion, "P25.53");
    assert.equal(policy.consumes.packageMaterializationApprovalAuditAccessPolicy.auditRecordRead, false);
    assert.equal(policy.consumes.packageMaterializationApprovalAuditAccessPolicy.accessGranted, false);
    assert.equal(policy.consumes.packageMaterializationFinalApprovalChecklist.checklistVersion, "P25.40");
...
function assertP25136AttestationNotarizationPolicyMetadataOnly(policy) {
    const capitalAttestationTopic = P25135_POLICY_KEY.replace(/^packageMaterializationApprovalAudit/, "").replace(/Policy$/, "");
    const attestationTopic = capitalAttestationTopic[0].toLowerCase() + capitalAttestationTopic.slice(1);
    const capitalNotarizationTopic = capitalAttestationTopic + "Notarization";
    const notarizationTopic = attestationTopic + "Notarization";
    const auditAttestationTopic = "audit" + capitalAttestationTopic;
    ...
    assert.ok(policy, "P25.136 policy payload");
    assert.equal(policy.status, "planned-disabled");
    assert.equal(policy[auditNotarizationTopic + "Version"], "P25.136");
    assert.equal(policy.adapter, "renderer-service");
    assert.equal(policy.command, "render.thumbnail");
    assert.equal(policy.dryRunOnly, true);
    assert.equal(policy.approvalRequired, true);
    ...
    assert.equal(policy.consumes[P25135_POLICY_KEY].currentStatus, "planned-disabled");
    assert.equal(policy.consumes[P25135_POLICY_KEY][auditAttestationTopic + "Version"], "P25.135");
    assert.equal(policy.consumes[P25135_POLICY_KEY][attestationTopic + "Created"], false);
    assert.equal(policy.consumes[P25135_POLICY_KEY][attestationTopic + "Stored"], false);
    assert.equal(policy.consumes[P25135_POLICY_KEY][attestationTopic + "RecordCreated"], false);
    assert.equal(policy.consumes[P25135_POLICY_KEY][attestationTopic + "RecordStored"], false);
    assert.equal(policy.consumes[P25135_POLICY_KEY][attestationTopic + "Notarized"], false);
    assert.equal(policy.consumes.packageMaterializationApprovalAuditAccessPolicy.auditAccessVersion, "P25.53");
    assert.equal(policy.consumes.packageMaterializationApprovalAuditAccessPolicy.auditRecordRead, false);
    assert.equal(policy.consumes.packageMaterializationApprovalAuditAccessPolicy.accessGranted, false);
    assert.equal(policy.consumes.packageMaterializationFinalApprovalChecklist.checklistVersion, "P25.40");
...
function assertP25137AttestationNotarizationCertificationPolicyMetadataOnly(policy) {
    const capitalNotarizationTopic = P25136_POLICY_KEY.replace(/^packageMaterializationApprovalAudit/, "").replace(/Policy$/, "");
    const notarizationTopic = capitalNotarizationTopic[0].toLowerCase() + capitalNotarizationTopic.slice(1);
    const capitalCertificationTopic = capitalNotarizationTopic + "Certification";
    const certificationTopic = notarizationTopic + "Certification";
    const capitalAttestationTopic = capitalNotarizationTopic.replace(/Notarization$/, "");
    ...
    assert.ok(policy, "P25.137 policy payload");
    assert.equal(policy.status, "planned-disabled");
    assert.equal(policy[auditCertificationTopic + "Version"], "P25.137");
    assert.equal(policy.adapter, "renderer-service");
    assert.equal(policy.command, "render.thumbnail");
    assert.equal(policy.dryRunOnly, true);
    assert.equal(policy.approvalRequired, true);
    ...
    assert.equal(policy.consumes[P25136_POLICY_KEY].currentStatus, "planned-disabled");
    assert.equal(policy.consumes[P25136_POLICY_KEY][auditNotarizationTopic + "Version"], "P25.136");
    assert.equal(policy.consumes[P25136_POLICY_KEY][notarizationTopic + "Created"], false);
    assert.equal(policy.consumes[P25136_POLICY_KEY][notarizationTopic + "Stored"], false);
    assert.equal(policy.consumes[P25136_POLICY_KEY][notarizationTopic + "RecordRead"], false);
    assert.equal(policy.consumes[P25136_POLICY_KEY][notarizationTopic + "Certified"], false);
    assert.equal(policy.consumes.packageMaterializationApprovalAuditAccessPolicy.auditAccessVersion, "P25.53");
    assert.equal(policy.consumes.packageMaterializationApprovalAuditAccessPolicy.auditRecordRead, false);
    assert.equal(policy.consumes.packageMaterializationApprovalAuditAccessPolicy.accessGranted, false);
    assert.equal(policy.consumes.packageMaterializationFinalApprovalChecklist.checklistVersion, "P25.40");
...
function assertP25138AttestationNotarizationCertificationEndorsementPolicyMetadataOnly(policy) {
    ...
    assert.ok(policy[auditEndorsementTopic + "Checks"].some((check) => check.id === "audit-access-granted" && check.executed === false));
    assert.ok(policy[auditEndorsementTopic + "Checks"].some((check) => check.id.endsWith("-record-present") && check.executed === false));
    assert.ok(policy[auditEndorsementTopic + "Checks"].some((check) => check.id.endsWith("-not-created") && check.executed === false));
    assert.ok(policy[auditEndorsementTopic + "Checks"].some((check) => check.id.endsWith("-record-not-stored") && check.executed === false));
    assert.equal(policy[auditEndorsementTopic + "Decision"].status, "blocked");
...
function assertP25139AttestationNotarizationCertificationEndorsementCountersignaturePolicyMetadataOnly(policy) {
...
function assertP25140AttestationNotarizationCertificationEndorsementCountersignatureVerificationPolicyMetadataOnly(policy) {
    const capitalCountersignatureTopic = P25139_POLICY_KEY.replace(/^packageMaterializationApprovalAudit/, "").replace(/Policy$/, "");
    const countersignatureTopic = capitalCountersignatureTopic[0].toLowerCase() + capitalCountersignatureTopic.slice(1);
    const capitalVerificationTopic = capitalCountersignatureTopic + "Verification";
    const verificationTopic = countersignatureTopic + "Verification";
    const auditCountersignatureTopic = "audit" + capitalCountersignatureTopic;
    ...
    assert.ok(policy, "P25.140 policy payload");
    assert.equal(policy.status, "planned-disabled");
    assert.equal(policy[auditVerificationTopic + "Version"], "P25.140");
    assert.equal(policy.adapter, "renderer-service");
    assert.equal(policy.command, "render.thumbnail");
    assert.equal(policy.dryRunOnly, true);
    assert.equal(policy.approvalRequired, true);
    ...
    assert.equal(policy.consumes[P25139_POLICY_KEY].currentStatus, "planned-disabled");
    assert.equal(policy.consumes[P25139_POLICY_KEY][auditCountersignatureTopic + "Version"], "P25.139");
    assert.equal(policy.consumes[P25139_POLICY_KEY][countersignatureTopic + "Created"], false);
    assert.equal(policy.consumes[P25139_POLICY_KEY][countersignatureTopic + "Stored"], false);
    assert.equal(policy.consumes[P25139_POLICY_KEY][countersignatureTopic + "RecordStored"], false);
    assert.equal(policy.consumes[P25139_POLICY_KEY][countersignatureTopic + "RecordRead"], false);
    assert.equal(policy.consumes[P25139_POLICY_KEY][countersignatureTopic + "Verified"], false);
    assert.equal(policy.consumes.packageMaterializationApprovalAuditAccessPolicy.auditAccessVersion, "P25.53");
    assert.equal(policy.consumes.packageMaterializationApprovalAuditAccessPolicy.auditRecordRead, false);
    assert.equal(policy.consumes.packageMaterializationApprovalAuditAccessPolicy.accessGranted, false);
    assert.equal(policy.consumes.packageMaterializationFinalApprovalChecklist.checklistVersion, "P25.40");
...
function assertP25141AttestationNotarizationCertificationEndorsementCountersignatureVerificationRevocationPolicyMetadataOnly(policy) {
    const capitalVerificationTopic = P25140_POLICY_KEY.replace(/^packageMaterializationApprovalAudit/, "").replace(/Policy$/, "");
    const verificationTopic = capitalVerificationTopic[0].toLowerCase() + capitalVerificationTopic.slice(1);
    const capitalRevocationTopic = capitalVerificationTopic + "Revocation";
    const revocationTopic = verificationTopic + "Revocation";
    const capitalCountersignatureTopic = capitalVerificationTopic.replace(/Verification$/, "");
    ...
    assert.ok(policy, "P25.141 policy payload");
    assert.equal(policy.status, "planned-disabled");
    assert.equal(policy[auditRevocationTopic + "Version"], "P25.141");
    assert.equal(policy.adapter, "renderer-service");
    assert.equal(policy.command, "render.thumbnail");
    assert.equal(policy.dryRunOnly, true);
    assert.equal(policy.approvalRequired, true);
    ...
    assert.equal(policy.consumes[P25140_POLICY_KEY].currentStatus, "planned-disabled");
    assert.equal(policy.consumes[P25140_POLICY_KEY][auditVerificationTopic + "Version"], "P25.140");
    assert.equal(policy.consumes[P25140_POLICY_KEY][verificationTopic + "Executed"], false);
    assert.equal(policy.consumes[P25140_POLICY_KEY][verificationTopic + "Stored"], false);
    assert.equal(policy.consumes[P25140_POLICY_KEY][verificationTopic + "RecordStored"], false);
    assert.equal(policy.consumes[P25140_POLICY_KEY][verificationTopic + "RecordRead"], false);
    assert.equal(policy.consumes[P25140_POLICY_KEY][verificationTopic + "Passed"], false);
    assert.equal(policy.consumes[P25140_POLICY_KEY][verificationTopic + "Verified"], false);
    assert.equal(policy.consumes[P25140_POLICY_KEY][verificationTopic + "Revoked"], false);
    assert.equal(policy.consumes.packageMaterializationApprovalAuditAccessPolicy.auditAccessVersion, "P25.53");
    assert.equal(policy.consumes.packageMaterializationApprovalAuditAccessPolicy.auditRecordRead, false);
    assert.equal(policy.consumes.packageMaterializationApprovalAuditAccessPolicy.accessGranted, false);
    assert.equal(policy.consumes.packageMaterializationFinalApprovalChecklist.checklistVersion, "P25.40");
...
function assertP25142AttestationNotarizationCertificationEndorsementCountersignatureVerificationRevocationAppealPolicyMetadataOnly(policy) {
    const capitalRevocationTopic = P25141_POLICY_KEY.replace(/^packageMaterializationApprovalAudit/, "").replace(/Policy$/, "");
    const revocationTopic = capitalRevocationTopic[0].toLowerCase() + capitalRevocationTopic.slice(1);
    const capitalAppealTopic = capitalRevocationTopic + "Appeal";
    const appealTopic = revocationTopic + "Appeal";
    const auditRevocationTopic = "audit" + capitalRevocationTopic;
    ...
    assert.ok(policy, "P25.142 policy payload");
    assert.equal(policy.status, "planned-disabled");
    assert.equal(policy[auditAppealTopic + "Version"], "P25.142");
    assert.equal(policy.adapter, "renderer-service");
    assert.equal(policy.command, "render.thumbnail");
    assert.equal(policy.dryRunOnly, true);
    assert.equal(policy.approvalRequired, true);
    ...
    assert.equal(policy.consumes[P25141_POLICY_KEY].currentStatus, "planned-disabled");
    assert.equal(policy.consumes[P25141_POLICY_KEY][auditRevocationTopic + "Version"], "P25.141");
    assert.equal(policy.consumes[P25141_POLICY_KEY][revocationTopic + "Revoked"], false);
    assert.equal(policy.consumes[P25141_POLICY_KEY][revocationTopic + "RecordStored"], false);
    assert.equal(policy.consumes[P25141_POLICY_KEY][revocationTopic + "RecordRead"], false);
    assert.equal(policy.consumes.packageMaterializationApprovalAuditAccessPolicy.auditAccessVersion, "P25.53");
    assert.equal(policy.consumes.packageMaterializationApprovalAuditAccessPolicy.auditRecordRead, false);
    assert.equal(policy.consumes.packageMaterializationApprovalAuditAccessPolicy.accessGranted, false);
    assert.equal(policy.consumes.packageMaterializationFinalApprovalChecklist.checklistVersion, "P25.40");
    ...
    assert.ok(policy[auditAppealTopic + "Checks"].some((check) => check.id === "audit-access-granted" && check.executed === false));
    assert.ok(policy[auditAppealTopic + "Checks"].some((check) => check.id.endsWith("-record-present") && check.executed === false));
    assert.ok(policy[auditAppealTopic + "Checks"].some((check) => check.id.endsWith("-appeal-policy-selected") && check.executed === false));
    assert.ok(policy[auditAppealTopic + "Checks"].some((check) => check.id.endsWith("-not-appealed") && check.executed === false));
    assert.ok(policy[auditAppealTopic + "Checks"].some((check) => check.id.endsWith("-appeal-record-not-created") && check.executed === false));
    assert.ok(policy[auditAppealTopic + "Checks"].some((check) => check.id.endsWith("-appeal-record-not-stored") && check.executed === false));
    assert.equal(policy[auditAppealTopic + "Decision"].status, "blocked");
    assert.equal(policy[auditAppealTopic + "Decision"]["canSelect" + capitalAppealTopic + "Policy"], false);
    assert.equal(policy[auditAppealTopic + "Decision"]["canCapture" + capitalAppealTopic + "Reason"], false);
    assert.equal(policy[auditAppealTopic + "Decision"]["canExecute" + capitalAppealTopic], false);
    ...
    assert.equal(policy[auditAppealTopic + "Decision"]["canRead" + capitalRevocationTopic + "Record"], false);
    assert.equal(policy[auditAppealTopic + "Decision"].canReadAuditRecord, false);
    assert.equal(policy[auditAppealTopic + "Decision"].canEnableRuntimeDispatch, false);
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not capture appeal reasons or compute appeal scopes")));
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not appeal countersignature verification revocations")));
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not create, store, or publish appeal records")));
    assert.ok(policy.noOpGuarantees.some((item) => item.includes("does not register runtime dispatch")));
    assert.ok(policy.requiredBeforeRuntimeDispatch.includes("keep render.thumbnail unavailable until executable adapter registration is approved"));
}


function assertP25143AttestationNotarizationCertificationEndorsementCountersignatureVerificationRevocationAppealResolutionPolicyMetadataOnly(policy) {
    const capitalAppealTopic = P25142_POLICY_KEY.replace(/^packageMaterializationApprovalAudit/, "").replace(/Policy$/, "");
    const appealTopic = capitalAppealTopic[0].toLowerCase() + capitalAppealTopic.slice(1);
    const capitalRevocationTopic = capitalAppealTopic.replace(/Appeal$/, "");
    const revocationTopic = appealTopic.replace(/Appeal$/, "");
    const capitalResolutionTopic = capitalAppealTopic + "Resolution";
    ...
    assert.ok(policy, "P25.143 policy payload");
    assert.equal(policy.status, "planned-disabled");
    assert.equal(policy[auditResolutionTopic + "Version"], "P25.143");
    assert.equal(policy.adapter, "renderer-service");
    assert.equal(policy.command, "render.thumbnail");
    assert.equal(policy.dryRunOnly, true);
    assert.equal(policy.approvalRequired, true);
    ...
    assert.equal(policy.consumes[P25142_POLICY_KEY].currentStatus, "planned-disabled");
    assert.equal(policy.consumes[P25142_POLICY_KEY][auditAppealTopic + "Version"], "P25.142");
    assert.equal(policy.consumes[P25142_POLICY_KEY][revocationTopic + "Appealed"], false);
    assert.equal(policy.consumes[P25142_POLICY_KEY][appealTopic + "RecordStored"], false);
    assert.equal(policy.consumes[P25142_POLICY_KEY][appealTopic + "RecordRead"], false);
    assert.equal(policy.consumes[P25142_POLICY_KEY][appealTopic + "Resolved"], false);
    assert.equal(policy.consumes.packageMaterializationApprovalAuditAccessPolicy.auditAccessVersion, "P25.53");
    assert.equal(policy.consumes.packageMaterializationApprovalAuditAccessPolicy.auditRecordRead, false);
    assert.equal(policy.consumes.packageMaterializationApprovalAuditAccessPolicy.accessGranted, false);
    assert.equal(policy.consumes.packageMaterializationFinalApprovalChecklist.checklistVersion, "P25.40");
...
function (policy) {
    ...
    assert.equal(policy[`${auditCountersignatureTopic}Decision`].canEnableRuntimeDispatch, false);
    assert.ok(policy.noOpGuarantees.some((entry) => entry.includes("does not read, countersign, or verify endorsements")));
}
...
function assertAuditCountersignatureRevocationAppealResolutionEnforcementEvidenceAttestationNotarizationCertificationEndorsementCountersignatureVerificationRevocationAppealResolutionEnforcementEvidenceAttestationNotarizationCertificationPolicyMetadataOnly(policy) {
    ...
    const capitalEnforcementTopic = `${capitalResolutionTopic}Enforcement`;
    const capitalEvidenceTopic = `${capitalEnforcementTopic}Evidence`;
    const capitalNotarizationTopic = `${capitalEvidenceTopic}AttestationNotarization`;
    const capitalCertificationTopic = `${capitalNotarizationTopic}Certification`;
    const auditCertificationTopic = `audit${capitalCertificationTopic}`;
...
test("renderer-service status reports a no-spawn lifecycle plan without probing", async () => {
    ...
    try {
        ...
        assert.equal(body.data.lifecycle.bundledSceneBridgeModuleNamespaceImportPreflight.status, "planned-disabled");
        assert.equal(body.data.lifecycle.bundledSceneBridgeModuleNamespaceImportPreflight.preflightVersion, "P26.36");
        assert.equal(body.data.lifecycle.bundledSceneBridgeModuleNamespaceImportPreflight.checked, false);
        assert.equal(
            body.data.lifecycle.bundledSceneBridgeModuleNamespaceImportPreflight.source.readiness,
            "blocked-until-import-gate-opens"
        );
        assert.equal(body.data.lifecycle.bundledSceneBridgeModuleNamespaceImportPreflight.moduleImport.importAttempted, false);
        assert.equal(body.data.lifecycle.bundledSceneBridgeModuleNamespaceImportPreflight.moduleImport.moduleImported, false);
        assert.equal(body.data.lifecycle.bundledSceneBridgeModuleNamespaceImportPreflight.moduleImport.importSucceeded, false);
        assert.equal(body.data.lifecycle.bundledSceneBridgeModuleNamespaceImportPreflight.factoryShape.factoryInvoked, false);
        assert.equal(body.data.lifecycle.bundledSceneBridgeModuleNamespaceImportPreflight.runtimeOptionsShape.runtimeRegistration, false);
        assert.deepEqual(body.data.lifecycle.bundledSceneBridgeModuleNamespaceImportPreflight.diagnosticCodes, [
            "renderer_service_bundled_scene_bridge_module_namespace_import_gate_closed",
        ]);
        assert.equal(
            body.data.lifecycle.bundledSceneBridgeModuleNamespaceImportPreflight.diagnosticsSurface,
            "healthPreflight.bundledSceneBridgeModuleNamespaceImportPreflight"
        );
        assert.equal(body.data.lifecycle.bundledSceneBridgeModuleNamespaceImportPreflight.lifecyclePlanEffects.moduleImport, false);
        assert.equal(body.data.lifecycle.bundledSceneBridgeModuleNamespaceImportPreflight.lifecyclePlanEffects.namespaceInspection, false);
        assert.equal(body.data.lifecycle.bundledSceneBridgeModuleNamespaceImportPreflight.lifecyclePlanEffects.factoryInvoked, false);
        assert.equal(body.data.lifecycle.bundledSceneBridgeModuleNamespaceImportPreflight.lifecyclePlanEffects.runtimeOptionsCreated, false);
        assert.equal(body.data.lifecycle.bundledSceneBridgeModuleNamespaceImportPreflight.lifecyclePlanEffects.renderDispatch, false);
        assert.equal(body.data.lifecycle.bundledSceneBridgeModuleNamespaceImportPreflight.omitted.moduleNamespace, true);
        ...
        assert.equal(
            body.data.lifecycle.bundledSceneBridgeFactoryInvocationPreflight.source.readiness,
            "blocked-until-namespace-import-ready"
        );
        assert.equal(body.data.lifecycle.bundledSceneBridgeFactoryInvocationPreflight.guard.invocationEnabled, false);
        assert.equal(body.data.lifecycle.bundledSceneBridgeFactoryInvocationPreflight.guard.factoryInvoked, false);
        assert.equal(body.data.lifecycle.bundledSceneBridgeFactoryInvocationPreflight.factoryInvocation.factoryInvoked, false);
        assert.deepEqual(body.data.lifecycle.bundledSceneBridgeFactoryInvocationPreflight.inertOptionsPlan.requiredOptionKeys, [
            "assetManifest",
            "runtimeAssetPreflight",
            "browser",
        ]);
        assert.equal(body.data.lifecycle.bundledSceneBridgeFactoryInvocationPreflight.inertOptionsPlan.optionValuesCreated, false);
        assert.equal(body.data.lifecycle.bundledSceneBridgeFactoryInvocationPreflight.runtimeOptionsShape.runtimeOptionsCreated, false);
        assert.equal(body.data.lifecycle.bundledSceneBridgeFactoryInvocationPreflight.runtimeOptionsShape.runtimeRegistration, false);
        assert.deepEqual(body.data.lifecycle.bundledSceneBridgeFactoryInvocationPreflight.diagnosticCodes, [
            "renderer_service_bundled_scene_bridge_factory_invocation_namespace_not_ready",
        ]);
        assert.equal(
            body.data.lifecycle.bundledSceneBridgeFactoryInvocationPreflight.diagnosticsSurface,
            "healthPreflight.bundledSceneBridgeFactoryInvocationPreflight"
        );
        assert.equal(body.data.lifecycle.bundledSceneBridgeFactoryInvocationPreflight.lifecyclePlanEffects.factoryInvocation, false);
        assert.equal(body.data.lifecycle.bundledSceneBridgeFactoryInvocationPreflight.lifecyclePlanEffects.factoryInvoked, false);
        assert.equal(body.data.lifecycle.bundledSceneBridgeFactoryInvocationPreflight.lifecyclePlanEffects.runtimeOptionsCreated, false);
        ...
        assert.equal(body.data.lifecycle.bundledSceneBridgeRuntimeRegistryInstallationContract.status, "planned-disabled");
        assert.equal(body.data.lifecycle.bundledSceneBridgeRuntimeRegistryInstallationContract.contractVersion, "P26.42");
        assert.equal(body.data.lifecycle.bundledSceneBridgeRuntimeRegistryInstallationContract.checked, false);
        assert.equal(
            body.data.lifecycle.bundledSceneBridgeRuntimeRegistryInstallationContract.source.runtimeRegistrationPreflightVersion,
            "P26.40"
        );
        assert.equal(
            body.data.lifecycle.bundledSceneBridgeRuntimeRegistryInstallationContract.source.registryRegistrationBoundaryVersion,
            "P26.41"
        );
        assert.equal(
            body.data.lifecycle.bundledSceneBridgeRuntimeRegistryInstallationContract.source.registryRegistrationBoundaryReady,
            false
        );
        assert.equal(
            body.data.lifecycle.bundledSceneBridgeRuntimeRegistryInstallationContract.source.readiness,
            "blocked-until-registry-registration-boundary-ready"
        );
        assert.equal(
            body.data.lifecycle.bundledSceneBridgeRuntimeRegistryInstallationContract.guard.runtimeInstallationEnabled,
            false
        );
        assert.equal(body.data.lifecycle.bundledSceneBridgeRuntimeRegistryInstallationContract.guard.runtimeInstalled, false);
        assert.deepEqual(
            body.data.lifecycle.bundledSceneBridgeRuntimeRegistryInstallationContract.runtimeValueShape.requiredMethods,
            ["renderThumbnail"]
        );
        assert.equal(
            body.data.lifecycle.bundledSceneBridgeRuntimeRegistryInstallationContract.runtimeValueShape.runtimeValueInstalled,
            false
        );
        assert.equal(
            body.data.lifecycle.bundledSceneBridgeRuntimeRegistryInstallationContract.closeHookOwnership.closeHookRegistered,
            false
        );
        assert.equal(
            body.data.lifecycle.bundledSceneBridgeRuntimeRegistryInstallationContract.duplicateRollbackHandling.rollbackAttempted,
            false
        );
        assert.deepEqual(body.data.lifecycle.bundledSceneBridgeRuntimeRegistryInstallationContract.diagnosticCodes, [
            "renderer_service_bundled_scene_bridge_runtime_registry_installation_contract_boundary_not_ready",
        ]);
        assert.equal(
            body.data.lifecycle.bundledSceneBridgeRuntimeRegistryInstallationContract.diagnosticsSurface,
            "healthPreflight.bundledSceneBridgeRuntimeRegistryInstallationContract"
        );
        assert.equal(
            body.data.lifecycle.bundledSceneBridgeRuntimeRegistryInstallationContract.lifecyclePlanEffects.healthProbe,
            false
        );
        assert.equal(
            body.data.lifecycle.bundledSceneBridgeRuntimeRegistryInstallationContract.lifecyclePlanEffects.runtimeValueCreation,
            false
        );
        assert.equal(
            body.data.lifecycle.bundledSceneBridgeRuntimeRegistryInstallationContract.lifecyclePlanEffects.runtimeInstallation,
            false
        );
        assert.equal(
            body.data.lifecycle.bundledSceneBridgeRuntimeRegistryInstallationContract.lifecyclePlanEffects.closeHookRegistration,
            false
        );
        assert.equal(
            body.data.lifecycle.bundledSceneBridgeRuntimeRegistryInstallationContract.lifecyclePlanEffects.duplicateRollback,
            false
        );
        assert.equal(
            body.data.lifecycle.bundledSceneBridgeRuntimeRegistryInstallationContract.lifecyclePlanEffects.renderDispatch,
            false
        );
        assert.equal(
            body.data.lifecycle.bundledSceneBridgeRuntimeRegistryInstallationContract.lifecyclePlanEffects.runtimeValuesIncluded,
            false
        );
        ...
        const installationPreflight = body.data.lifecycle.bundledSceneBridgeRuntimeRegistryInstallationPreflight;
        assert.equal(installationPreflight.status, "blocked");
        assert.equal(installationPreflight.preflightVersion, "P26.44");
        assert.equal(installationPreflight.checked, false);
        assert.equal(installationPreflight.source.registryInstallationGateReady, false);
        assert.equal(installationPreflight.source.registryInstallationGateStatus, "planned-disabled");
        assert.equal(installationPreflight.source.readiness, "blocked-until-renderer-service-health");
        assert.equal(installationPreflight.preflight.readyForLaterInstallationTask, false);
        assert.equal(installationPreflight.preflight.registryLookupAttempted, false);
        assert.equal(installationPreflight.preflight.runtimeInstallationAttempted, false);
        assert.equal(installationPreflight.preflight.runtimeInstalled, false);
        assert.equal(installationPreflight.preflight.renderDispatch, false);
        assert.equal(installationPreflight.refusalDiagnostics.refusalReason, "renderer-service-health-not-checked");
        assert.equal(installationPreflight.refusalDiagnostics.valuesIncluded, false);
        assert.equal(installationPreflight.rollbackPreconditions.preconditionsVerified, false);
        assert.equal(installationPreflight.rollbackPreconditions.rollbackAttempted, false);
        assert.equal(installationPreflight.lifecycleOwnership.lifecycleOwnershipVerified, false);
        assert.equal(installationPreflight.lifecycleOwnership.runtimeValueOwned, false);
        assert.deepEqual(installationPreflight.diagnosticCodes, [
            "renderer_service_bundled_scene_bridge_runtime_registry_installation_preflight_health_not_checked",
        ]);
        assert.equal(
            installationPreflight.diagnosticsSurface,
            "healthPreflight.bundledSceneBridgeRuntimeRegistryInstallationPreflight"
        );
        assert.equal(installationPreflight.lifecyclePlanEffects.healthProbe, false);
        assert.equal(installationPreflight.lifecyclePlanEffects.registryLookup, false);
        assert.equal(installationPreflight.lifecyclePlanEffects.registryWrite, false);
        assert.equal(installationPreflight.lifecyclePlanEffects.runtimeValueCreation, false);
        assert.equal(installationPreflight.lifecyclePlanEffects.runtimeInstallation, false);
        assert.equal(installationPreflight.lifecyclePlanEffects.closeHookRegistration, false);
        assert.equal(installationPreflight.lifecyclePlanEffects.duplicateRollback, false);
        assert.equal(installationPreflight.lifecyclePlanEffects.renderDispatch, false);
        assert.equal(installationPreflight.lifecyclePlanEffects.backendRpcReads, false);
        assert.equal(installationPreflight.lifecyclePlanEffects.sourceDataReads, false);
        assert.equal(installationPreflight.lifecyclePlanEffects.runtimeValuesIncluded, false);
        assert.equal(installationPreflight.lifecyclePlanEffects.registryValuesIncluded, false);
        assert.equal(installationPreflight.omitted.configuredValue, true);
        assert.equal(installationPreflight.omitted.runtimeValue, true);
        assert.equal(installationPreflight.omitted.registryValue, true);
        assert.equal(installationPreflight.omitted.lifecycleHandles, true);
...
test("renderer-service status text reports bundled scene bridge registry installation contract", async () => {
    const result = await runCli(["renderer-service", "status", "--host", "127.0.0.1", "--port", "6072"]);

    assert.equal(result.exitCode, 0);
    assert.match(
        result.stdout,
        /Bundled scene bridge runtime registry installation contract: planned-disabled/
    );
    assert.match(
        result.stdout,
    ...
    assert.match(
        result.stdout,
        /Bundled scene bridge runtime registry installation preflight: blocked/
    );
    assert.match(
        result.stdout,
        /Bundled scene bridge runtime registry installation preflight diagnostics: renderer_service_bundled_scene_bridge_runtime_registry_installation_preflight_health_not_checked/
    );
    assert.match(result.stdout, /Runtime registry installation preflight next actions:/);
});
...
test("renderer-service status reports bundled scene bridge registration preflight gate configuration", async () => {
    ...
    assert.equal(installationContract.status, "planned-disabled");
    assert.equal(installationContract.contractVersion, "P26.42");
    assert.equal(installationContract.source.registryRegistrationBoundaryReady, false);
    assert.equal(installationContract.guard.runtimeInstallationEnabled, false);
    assert.equal(installationContract.guard.runtimeInstalled, false);
    assert.equal(installationContract.runtimeValueShape.runtimeValueInstalled, false);
    assert.equal(installationContract.closeHookOwnership.closeHookRegistered, false);
    assert.equal(installationContract.duplicateRollbackHandling.rollbackAttempted, false);
    assert.equal(installationContract.lifecyclePlanEffects.healthProbe, false);
    assert.equal(installationContract.lifecyclePlanEffects.runtimeValueCreation, false);
    assert.equal(installationContract.lifecyclePlanEffects.runtimeInstallation, false);
    assert.equal(installationContract.lifecyclePlanEffects.closeHookRegistration, false);
    assert.equal(installationContract.lifecyclePlanEffects.duplicateRollback, false);
    assert.equal(installationContract.lifecyclePlanEffects.renderDispatch, false);
    assert.equal(installationContract.omitted.runtimeValue, true);
    assert.ok(
    ...
    assert.equal(installationPreflight.omitted.runtimeValue, true);
    assert.ok(
        installationPreflight.nextActions.some((entry) => entry.includes("bundledSceneBridgeRuntimeRegistryInstallationPreflight"))
    );
    assert.match(
        body.data.lifecycle.startCommand,
        /PENPOT_RENDERER_SERVICE_BUNDLED_SCENE_BRIDGE_RUNTIME=registration-preflight/
...
test("renderer-service status reports reviewed runtime registry installation gate configuration without assuming health readiness", async () => {
    ...
    assert.equal(installationPreflight.source.registryInstallationGateReady, false);
    assert.equal(installationPreflight.source.registryInstallationGateStatus, "planned-disabled");
    assert.equal(installationPreflight.source.reviewedGateOpen, false);
    assert.equal(installationPreflight.source.futureInstallationAttemptAllowed, false);
    assert.equal(installationPreflight.source.readiness, "blocked-until-renderer-service-health");
    assert.equal(installationPreflight.preflight.readyForLaterInstallationTask, false);
    assert.equal(installationPreflight.preflight.runtimeInstalled, false);
    assert.equal(installationPreflight.lifecyclePlanEffects.healthProbe, false);
...
test("renderer-service status reports invalid runtime registry installation gate configuration", async () => {
    ...
    assert.equal(installationPreflight.status, "invalid");
    assert.equal(installationPreflight.source.registryInstallationGateStatus, "invalid");
    assert.equal(installationPreflight.source.readiness, "invalid-installation-gate");
    assert.equal(installationPreflight.refusalDiagnostics.invalidGateMetadata, true);
    assert.equal(installationPreflight.refusalDiagnostics.valuesIncluded, false);
    assert.deepEqual(installationPreflight.diagnosticCodes, [
    ...
    assert.equal(installationPreflight.lifecyclePlanEffects.runtimeInstallation, false);
    assert.equal(installationPreflight.omitted.configuredValue, true);
    assert.equal(JSON.stringify(installationPreflight).includes("enabled"), false);
});
...
test("renderer-service start keeps startup manual and does not spawn a process", async () => {
    ...
    assert.equal(body.error.data.lifecycle.bundledSceneBridgeRuntimeRegistryInstallationContract.status, "planned-disabled");
    assert.equal(
        body.error.data.lifecycle.bundledSceneBridgeRuntimeRegistryInstallationContract.lifecyclePlanEffects.runtimeInstallation,
        false
    );
    assert.equal(
        body.error.data.lifecycle.bundledSceneBridgeRuntimeRegistryInstallationContract.lifecyclePlanEffects.closeHookRegistration,
        false
    );
    assert.equal(
        body.error.data.lifecycle.bundledSceneBridgeRuntimeRegistryInstallationContract.lifecyclePlanEffects.duplicateRollback,
        false
    );
    ...
    assert.equal(
        body.error.data.lifecycle.bundledSceneBridgeRuntimeRegistryInstallationPreflight.preflight.readyForLaterInstallationTask,
        false
    );
    assert.equal(
        body.error.data.lifecycle.bundledSceneBridgeRuntimeRegistryInstallationPreflight.lifecyclePlanEffects.registryLookup,
        false
    );
    assert.equal(
        body.error.data.lifecycle.bundledSceneBridgeRuntimeRegistryInstallationPreflight.lifecyclePlanEffects.runtimeInstallation,
        false
    );
    assert.equal(
        body.error.data.lifecycle.bundledSceneBridgeRuntimeRegistryInstallationPreflight.lifecyclePlanEffects.renderDispatch,
        false
    );
    assert.equal(body.error.data.lifecycle.bundledSceneBridgeRuntimeRegistryInstallationPreflight.omitted.runtimeValue, true);
...
test("render thumbnail dry-run returns renderer-service request plan", async () => {
    ...
    try {
        ...
        assertP25128RevocationAppealResolutionEnforcementEvidenceAttestationNotarizationCertificationEndorsementCountersignaturePolicyMetadataOnly(body.data[P25128_POLICY_KEY]);
        assertP25129RevocationAppealResolutionEnforcementEvidenceAttestationNotarizationCertificationEndorsementCountersignatureVerificationPolicyMetadataOnly(body.data[P25129_POLICY_KEY]);
        assertP25130RevocationAppealResolutionEnforcementEvidenceAttestationNotarizationCertificationEndorsementCountersignatureVerificationRevocationPolicyMetadataOnly(body.data[P25130_POLICY_KEY]);
        assertP25131RevocationAppealResolutionEnforcementEvidenceAttestationNotarizationCertificationEndorsementCountersignatureVerificationRevocationAppealPolicyMetadataOnly(body.data[P25131_POLICY_KEY]);
        assertP25132RevocationAppealResolutionEnforcementEvidenceAttestationNotarizationCertificationEndorsementCountersignatureVerificationRevocationAppealResolutionPolicyMetadataOnly(body.data[P25132_POLICY_KEY]);
        assertP25133RevocationAppealResolutionEnforcementEvidenceAttestationNotarizationCertificationEndorsementCountersignatureVerificationRevocationAppealResolutionEnforcementPolicyMetadataOnly(body.data[P25133_POLICY_KEY]);
        assertP25134RevocationAppealResolutionEnforcementEvidenceAttestationNotarizationCertificationEndorsementCountersignatureVerificationRevocationAppealResolutionEnforcementEvidencePolicyMetadataOnly(body.data[P25134_POLICY_KEY]);
        ...
        assertP25140AttestationNotarizationCertificationEndorsementCountersignatureVerificationPolicyMetadataOnly(body.data[P25140_POLICY_KEY]);
        assertP25141AttestationNotarizationCertificationEndorsementCountersignatureVerificationRevocationPolicyMetadataOnly(body.data[P25141_POLICY_KEY]);
        assertP25142AttestationNotarizationCertificationEndorsementCountersignatureVerificationRevocationAppealPolicyMetadataOnly(body.data[P25142_POLICY_KEY]);
        assertP25143AttestationNotarizationCertificationEndorsementCountersignatureVerificationRevocationAppealResolutionPolicyMetadataOnly(body.data[P25143_POLICY_KEY]);
...
test("render thumbnail execution opt-in posts to renderer-service and returns resource metadata", async () => {
    ...
    try {
        ...
        assert.equal(body.data.healthPreflight.bundledSceneBridgeContract.assetPrerequisites.approvalRequired, true);
        assert.equal(body.data.healthPreflight.bundledSceneBridgeContract.browserPageHandoff.browserProcessStarted, false);
        assert.equal(body.data.healthPreflight.bundledSceneBridgeContract.renderInputContract.sourceDataValuesIncluded, false);
        assert.equal(body.data.healthPreflight.bundledSceneBridgeContract.renderOutputContract.localFileWrites, false);
        ...
        assert.equal(body.data.healthPreflight.bundledSceneBridgeRuntimeRegistryInstallationContract.status, "planned-disabled");
        assert.equal(body.data.healthPreflight.bundledSceneBridgeRuntimeRegistryInstallationContract.contractVersion, "P26.42");
        assert.equal(
            body.data.healthPreflight.bundledSceneBridgeRuntimeRegistryInstallationContract.source.registryRegistrationBoundaryReady,
            false
        );
        assert.equal(
            body.data.healthPreflight.bundledSceneBridgeRuntimeRegistryInstallationContract.source.readiness,
            "blocked-until-registry-registration-boundary-ready"
        );
        assert.equal(
            body.data.healthPreflight.bundledSceneBridgeRuntimeRegistryInstallationContract.guard.runtimeInstallationEnabled,
            false
        );
        assert.equal(
            body.data.healthPreflight.bundledSceneBridgeRuntimeRegistryInstallationContract.guard.runtimeInstalled,
            false
        );
        assert.equal(
            body.data.healthPreflight.bundledSceneBridgeRuntimeRegistryInstallationContract.runtimeValueShape.runtimeValueInstalled,
            false
        );
        assert.equal(
            body.data.healthPreflight.bundledSceneBridgeRuntimeRegistryInstallationContract.closeHookOwnership.closeHookRegistered,
            false
        );
        assert.equal(
            body.data.healthPreflight.bundledSceneBridgeRuntimeRegistryInstallationContract.duplicateRollbackHandling.rollbackAttempted,
            false
        );
        assert.deepEqual(
            body.data.healthPreflight.bundledSceneBridgeRuntimeRegistryInstallationContract.diagnosticCodes,
            ["renderer_service_bundled_scene_bridge_runtime_registry_installation_contract_boundary_not_ready"]
        );
        assert.equal(
            body.data.healthPreflight.bundledSceneBridgeRuntimeRegistryInstallationContract.sideEffects.runtimeValueCreation,
            false
        );
        assert.equal(
            body.data.healthPreflight.bundledSceneBridgeRuntimeRegistryInstallationContract.sideEffects.runtimeInstallation,
            false
        );
        assert.equal(
            body.data.healthPreflight.bundledSceneBridgeRuntimeRegistryInstallationContract.sideEffects.closeHookRegistration,
            false
        );
        assert.equal(
            body.data.healthPreflight.bundledSceneBridgeRuntimeRegistryInstallationContract.sideEffects.duplicateRollback,
            false
        );
        assert.equal(
            body.data.healthPreflight.bundledSceneBridgeRuntimeRegistryInstallationContract.redaction.runtimeValuesIncluded,
            false
        );
        assert.equal(
        ...
        assert.equal(body.data.healthPreflight.bundledSceneBridgeRuntimeRegistryInstallationPreflight.status, "blocked");
        assert.equal(
            body.data.healthPreflight.bundledSceneBridgeRuntimeRegistryInstallationPreflight.preflightVersion,
            "P26.44"
        );
        assert.equal(body.data.healthPreflight.bundledSceneBridgeRuntimeRegistryInstallationPreflight.checked, true);
        assert.equal(
            body.data.healthPreflight.bundledSceneBridgeRuntimeRegistryInstallationPreflight.source.registryInstallationGateReady,
            false
        );
        assert.equal(
            body.data.healthPreflight.bundledSceneBridgeRuntimeRegistryInstallationPreflight.source.registryInstallationGateStatus,
            "planned-disabled"
        );
        assert.equal(
            body.data.healthPreflight.bundledSceneBridgeRuntimeRegistryInstallationPreflight.preflight.readyForLaterInstallationTask,
            false
        );
        assert.equal(
            body.data.healthPreflight.bundledSceneBridgeRuntimeRegistryInstallationPreflight.preflight.runtimeInstalled,
            false
        );
        assert.equal(
            body.data.healthPreflight.bundledSceneBridgeRuntimeRegistryInstallationPreflight.refusalDiagnostics.valuesIncluded,
            false
        );
        assert.equal(
            body.data.healthPreflight.bundledSceneBridgeRuntimeRegistryInstallationPreflight.rollbackPreconditions.preconditionsVerified,
            false
        );
        assert.equal(
            body.data.healthPreflight.bundledSceneBridgeRuntimeRegistryInstallationPreflight.rollbackPreconditions.rollbackAttempted,
            false
        );
        assert.equal(
            body.data.healthPreflight.bundledSceneBridgeRuntimeRegistryInstallationPreflight.lifecycleOwnership.runtimeValueOwned,
            false
        );
        assert.deepEqual(
            body.data.healthPreflight.bundledSceneBridgeRuntimeRegistryInstallationPreflight.diagnosticCodes,
            ["renderer_service_bundled_scene_bridge_runtime_registry_installation_preflight_gate_not_ready"]
        );
        assert.equal(
            body.data.healthPreflight.bundledSceneBridgeRuntimeRegistryInstallationPreflight.sideEffects.registryLookup,
            false
        );
        assert.equal(
            body.data.healthPreflight.bundledSceneBridgeRuntimeRegistryInstallationPreflight.sideEffects.registryWrite,
            false
        );
        assert.equal(
            body.data.healthPreflight.bundledSceneBridgeRuntimeRegistryInstallationPreflight.sideEffects.runtimeInstallation,
            false
        );
        assert.equal(
            body.data.healthPreflight.bundledSceneBridgeRuntimeRegistryInstallationPreflight.redaction.runtimeValuesIncluded,
            false
        );
        assert.equal(
            body.data.healthPreflight.bundledSceneBridgeRuntimeRegistryInstallationPreflight.omitted.configuredValue,
            true
        );
        assert.equal(
            body.data.healthPreflight.bundledSceneBridgeRuntimeRegistryInstallationPreflight.omitted.runtimeValue,
            true
        );
