...
function callerSessionAuthHeaders(token: string | undefined): Record<string, string> {
    if (!token) {
        return {};
    }
...
export class RenderThumbnailArgs {
    static schema = {
        fileId: uuidSchema.optional().describe("File id for dashboard file or tagged frame thumbnail rendering."),
        ...
        endpoint: z.string().url().optional().describe("Renderer-service thumbnail endpoint."),
        ...
        probeTimeoutMs: z.number().int().positive().max(60000).optional().describe("Renderer-service health probe timeout in milliseconds."),
        ...
        rendererServiceExecution: z
            ...
            .optional()
            .describe("Explicit opt-in value that enables gated renderer-service execution when dryRun is false."),
        publicUri: z.string().url().optional().describe("Public Penpot URI used to normalize relative resource download URIs."),
        output: z
            .string()
            .min(1)
            .optional()
            .describe("CLI-only output path metadata; MCP returns resource metadata without writing local files."),
        adapter: z.string().optional().describe("Optional adapter request: auto or renderer-service."),
        dryRun: z.boolean().optional().describe("Planning mode. Defaults to true; false executes the gated renderer-service path."),
    };
...
export class RenderThumbnailTool extends PenpotRpcTool<RenderThumbnailArgs> {
    ...
    protected async executeCore(args: RenderThumbnailArgs): Promise<ToolResponse> {
        ...
        if (args.dryRun === false) {
            try {
                const result = await this.executeRendererServicePlan(plan);
                return this.ok({
                    ...result,
                    adapterSelection,
                });
            } catch (cause) {
                const payload = createRenderThumbnailRendererServiceErrorPayload(plan, cause);
                return this.error(payload.code, payload.message, payload.actions, payload.data);
            }
        }
    ...
    private async executeRendererServicePlan(plan: RenderThumbnailRendererServicePlan): Promise<ReturnType<typeof createRenderThumbnailRendererServiceResult> & { healthPreflight: RenderThumbnailRendererServicePlan["healthPreflight"] }> {
        ...
        if (healthPreflight.status !== "ok") {
            throw {
                code: plan.executionGate.optIn.configured
        ...
        try {
            response = await fetch(plan.clientRequest.endpoint, {
                ...
                headers: {
                    ...
                    ...callerSessionAuthHeaders(this.getUserToken()),
                },
        ...
        let data: unknown;
        try {
            data = await this.readStructuredRendererServiceResponse(response);
        } catch (cause) {
            throw {
        ...
        return {
            ...createRenderThumbnailRendererServiceResult(plan, data, {
    ...
    private async readStructuredRendererServiceResponse(response: Response): Promise<unknown> {
        const text = await response.text();
        if (!text.trim()) {
            return null;
        }
    ...
    private adapterSelectionFailure(selection: CommandAdapterSelection): ToolResponse {
        const error = createAdapterSelectionError(selection, {
            actions: [
                "Use adapter: 'auto' or 'renderer-service' for the gated renderer-service thumbnail path.",
