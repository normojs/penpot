...
function thumbnailBackendRpcSummary(entry: any) {
...
function thumbnailServiceResponseFromRequest(serviceRequest: any, options: any = {}) {
...
const RUNTIME_ASSET_APPROVAL_UNSUPPORTED_DIAGNOSTIC_CODES = [
...
const RUNTIME_ASSET_APPROVAL_READY_BLOCKER_CODES = [
    "renderer_service_runtime_asset_materialization_approval_required",
    ...RUNTIME_ASSET_APPROVAL_UNSUPPORTED_DIAGNOSTIC_CODES,
...
function runtimeAssetMaterializationApprovalReadinessVerdictFixture() {
    return {
        status: "blocked",
        ...
        inputs: {
            sourceDryRun: {
                ...
                blockerCodes: ["renderer_service_runtime_asset_materialization_approval_required"],
            },
            approvalConfiguration: {
                configured: true,
                unsupportedConfiguration: true,
                unsupportedDiagnosticCodes: RUNTIME_ASSET_APPROVAL_UNSUPPORTED_DIAGNOSTIC_CODES,
                valuesIncluded: false,
            },
            approvalGate: {
                status: "closed",
                blockerCodes: RUNTIME_ASSET_APPROVAL_READY_BLOCKER_CODES,
        ...
        checks: [
            ...
            },
            {
                ...
                required: true,
                status: "blocked",
                diagnosticCodes: RUNTIME_ASSET_APPROVAL_UNSUPPORTED_DIAGNOSTIC_CODES,
            },
            {
...
function assertP25128RevocationAppealResolutionEnforcementEvidenceAttestationNotarizationCertificationEndorsementCountersignaturePolicyMetadataOnly(policy: any) {
...
function assertP25129RevocationAppealResolutionEnforcementEvidenceAttestationNotarizationCertificationEndorsementCountersignatureVerificationPolicyMetadataOnly(policy: any) {
...
function assertP25130RevocationAppealResolutionEnforcementEvidenceAttestationNotarizationCertificationEndorsementCountersignatureVerificationRevocationPolicyMetadataOnly(policy: any) {
...
function assertP25131RevocationAppealResolutionEnforcementEvidenceAttestationNotarizationCertificationEndorsementCountersignatureVerificationRevocationAppealPolicyMetadataOnly(policy: any) {
...
function assertP25132RevocationAppealResolutionEnforcementEvidenceAttestationNotarizationCertificationEndorsementCountersignatureVerificationRevocationAppealResolutionPolicyMetadataOnly(policy: any) {
...
function assertP25133RevocationAppealResolutionEnforcementEvidenceAttestationNotarizationCertificationEndorsementCountersignatureVerificationRevocationAppealResolutionEnforcementPolicyMetadataOnly(policy: any) {
...
function assertP25134RevocationAppealResolutionEnforcementEvidenceAttestationNotarizationCertificationEndorsementCountersignatureVerificationRevocationAppealResolutionEnforcementEvidencePolicyMetadataOnly(policy: any) {
...
function assertP25135RevocationAppealResolutionEnforcementEvidenceAttestationNotarizationCertificationEndorsementCountersignatureVerificationRevocationAppealResolutionEnforcementEvidenceAttestationPolicyMetadataOnly(policy: any) {
...
function assertP25136AttestationNotarizationPolicyMetadataOnly(policy: any) {
...
function assertP25137AttestationNotarizationCertificationPolicyMetadataOnly(policy: any) {
...
function assertP25140AttestationNotarizationCertificationEndorsementCountersignatureVerificationPolicyMetadataOnly(policy: any) {
...
function assertP25141AttestationNotarizationCertificationEndorsementCountersignatureVerificationRevocationPolicyMetadataOnly(policy: any) {
...
function assertP25142AttestationNotarizationCertificationEndorsementCountersignatureVerificationRevocationAppealPolicyMetadataOnly(policy: any) {
    ...
    assert.ok(policy[auditAppealTopic + "Checks"].some((check: PolicyCheck) => check.id === "audit-access-granted" && check.executed === false));
    assert.ok(policy[auditAppealTopic + "Checks"].some((check: PolicyCheck) => check.id.endsWith("-record-present") && check.executed === false));
    assert.ok(policy[auditAppealTopic + "Checks"].some((check: PolicyCheck) => check.id.endsWith("-appeal-policy-selected") && check.executed === false));
    assert.ok(policy[auditAppealTopic + "Checks"].some((check: PolicyCheck) => check.id.endsWith("-not-appealed") && check.executed === false));
    assert.ok(policy[auditAppealTopic + "Checks"].some((check: PolicyCheck) => check.id.endsWith("-appeal-record-not-created") && check.executed === false));
    assert.ok(policy[auditAppealTopic + "Checks"].some((check: PolicyCheck) => check.id.endsWith("-appeal-record-not-stored") && check.executed === false));
    assert.equal(policy[auditAppealTopic + "Decision"].status, "blocked");
    assert.equal(policy[auditAppealTopic + "Decision"]["canSelect" + capitalAppealTopic + "Policy"], false);
    assert.equal(policy[auditAppealTopic + "Decision"]["canCapture" + capitalAppealTopic + "Reason"], false);
    assert.equal(policy[auditAppealTopic + "Decision"]["canExecute" + capitalAppealTopic], false);
    ...
    assert.equal(policy[auditAppealTopic + "Decision"].canReadAuditRecord, false);
    assert.equal(policy[auditAppealTopic + "Decision"].canEnableRuntimeDispatch, false);
    assert.ok(policy.noOpGuarantees.some((item: string) => item.includes("does not capture appeal reasons or compute appeal scopes")));
    assert.ok(policy.noOpGuarantees.some((item: string) => item.includes("does not appeal countersignature verification revocations")));
    assert.ok(policy.noOpGuarantees.some((item: string) => item.includes("does not create, store, or publish appeal records")));
    assert.ok(policy.noOpGuarantees.some((item: string) => item.includes("does not register runtime dispatch")));
    assert.ok(policy.requiredBeforeRuntimeDispatch.includes("keep render.thumbnail unavailable until executable adapter registration is approved"));
}


function assertP25143AttestationNotarizationCertificationEndorsementCountersignatureVerificationRevocationAppealResolutionPolicyMetadataOnly(policy: any) {
...
test("RenderThumbnailTool dry-run returns renderer-service request metadata without network calls", async () => {
    ...
    try {
        ...
        assert.equal(body.data.executable, true);
...
test("RenderThumbnailTool execution opt-in posts to renderer-service and returns resource metadata", async () => {
    const originalFetch = globalThis.fetch;
                ...
                JSON.stringify({
                    ...
                    runtimeAssetMaterializationPreflight: {
                        ...
                        execution: {
                            ...
                            mode: "read-only",
                            readiness: "ready",
                            workspaceRoot: "/workspace/mcp-secret",
                            cacheRoot: "/cache/mcp-secret",
                            checks: [{ assetId: "thumbnail-worker-main", readiness: "ready", sha256: "abc123" }],
                            cacheOutputChecks: [{ cacheOutputId: "runtime-asset-cache", readiness: "ready" }],
                            summary: {
                                ready: true,
                                readyAssetIds: ["thumbnail-worker-main"],
                                missingAssetIds: [],
                    ...
                    runtimeAssetMaterializationApproval: {
                        ...
                        diagnosticCodes: RUNTIME_ASSET_APPROVAL_READY_BLOCKER_CODES,
                        nextActions: [
                            "Leave PENPOT_RENDERER_SERVICE_RUNTIME_ASSET_MATERIALIZATION_APPROVAL unset until the future approval gate is implemented.",
                            "Leave PENPOT_RENDERER_SERVICE_RUNTIME_ASSET_MATERIALIZATION_APPROVAL_TOKEN unset until approval token validation is implemented.",
                            "Leave PENPOT_RENDERER_SERVICE_RUNTIME_ASSET_MATERIALIZATION_APPROVAL_AUDIT_DIR unset until approval audit persistence is implemented.",
                        ...
                        readinessVerdict: runtimeAssetMaterializationApprovalReadinessVerdictFixture(),
                        sideEffects: {
                            browserProcessStarted: false,
                            runtimeExecutionRegistered: false,
                            runtimeAdapterImported: false,
    ...
    try {
        const tool = new RenderThumbnailTool(
            mcpServerWithSse(async () => {
                sseCalled = true;
                throw new Error("render.thumbnail health preflight must not call backend SSE");
            })
        );
        ...
        assert.equal(calls[1].init.headers.cookie, "auth-token=token-1");
        ...
        assert.deepEqual(
            body.data.healthPreflight.runtimeAssetMaterializationApproval.diagnostics.map((entry: any) => entry.code),
            [
                "renderer_service_runtime_asset_materialization_approval_configuration_unsupported",
                "renderer_service_runtime_asset_materialization_approval_token_unsupported",
                "renderer_service_runtime_asset_materialization_approval_audit_unsupported",
        ...
        assert.ok(
            body.data.healthPreflight.runtimeAssetMaterializationApproval.diagnosticCodes.includes(
                "renderer_service_runtime_asset_materialization_approval_token_unsupported"
            )
        );
        assert.equal(body.data.healthPreflight.runtimeAssetMaterializationApproval.diagnostics[1].valueRead, false);
        assert.equal(body.data.healthPreflight.runtimeAssetMaterializationApproval.diagnostics[1].valuesIncluded, false);
        assert.ok(
            body.data.healthPreflight.runtimeAssetMaterializationApproval.nextActions.some((entry: string) =>
                entry.includes("PENPOT_RENDERER_SERVICE_RUNTIME_ASSET_MATERIALIZATION_APPROVAL_TOKEN")
            )
        );
        const approvalVerdict = body.data.healthPreflight.runtimeAssetMaterializationApproval.readinessVerdict;
        assert.equal(approvalVerdict.status, "blocked");
        assert.equal(approvalVerdict.verdictVersion, "P26.29");
        assert.equal(approvalVerdict.computed, true);
        assert.equal(approvalVerdict.trusted, false);
        assert.equal(approvalVerdict.approvalReady, false);
        assert.equal(approvalVerdict.materializationReady, false);
        assert.equal(approvalVerdict.approvalGranted, false);
        assert.equal(approvalVerdict.writesEnabled, false);
        assert.equal(approvalVerdict.inputs.sourceDryRun.readiness, "ready");
        assert.equal(approvalVerdict.inputs.sourceDryRun.ready, true);
        assert.equal(approvalVerdict.inputs.sourceDryRun.copyPlanCounts.ready, 1);
        assert.deepEqual(approvalVerdict.inputs.sourceDryRun.blockerCodes, [
            "renderer_service_runtime_asset_materialization_approval_required",
        ]);
        assert.equal(approvalVerdict.inputs.approvalConfiguration.configured, true);
        assert.equal(approvalVerdict.inputs.approvalConfiguration.unsupportedConfiguration, true);
        assert.deepEqual(
            approvalVerdict.inputs.approvalConfiguration.unsupportedDiagnosticCodes,
            RUNTIME_ASSET_APPROVAL_UNSUPPORTED_DIAGNOSTIC_CODES
        );
        assert.equal(approvalVerdict.inputs.approvalConfiguration.valuesIncluded, false);
        assert.equal(approvalVerdict.inputs.approvalGate.status, "closed");
        assert.deepEqual(approvalVerdict.inputs.approvalGate.blockerCodes, RUNTIME_ASSET_APPROVAL_READY_BLOCKER_CODES);
        assert.equal(approvalVerdict.inputs.approvalGate.approvalRequired, true);
        assert.equal(approvalVerdict.inputs.approvalGate.approvalGranted, false);
        assert.equal(approvalVerdict.inputs.approvalGate.writesEnabled, false);
        assert.ok(
            approvalVerdict.checks.some(
                (entry: any) => entry.id === "runtime-asset-materialization-dry-run-ready" && entry.status === "passed"
            )
        );
        assert.ok(
            approvalVerdict.checks.some(
                (entry: any) =>
                    entry.id === "runtime-asset-materialization-approval-configuration-supported" &&
                    entry.status === "blocked" &&
                    entry.diagnosticCodes.includes("renderer_service_runtime_asset_materialization_approval_token_unsupported")
            )
        );
        assert.deepEqual(approvalVerdict.blockerCodes, RUNTIME_ASSET_APPROVAL_READY_BLOCKER_CODES);
        assert.ok(
            approvalVerdict.nextActions.some((entry: string) =>
                entry.includes("approval scaffold, token validation, and audit persistence")
            )
        );
        assert.equal(approvalVerdict.sideEffects.approvalTokenRead, false);
        assert.equal(approvalVerdict.sideEffects.approvalTokenAccepted, false);
        assert.equal(approvalVerdict.sideEffects.approvalTokenConsumed, false);
        assert.equal(approvalVerdict.sideEffects.auditRecordWritten, false);
        assert.equal(approvalVerdict.sideEffects.localFileWrites, false);
        assert.equal(approvalVerdict.sideEffects.networkDispatch, false);
        assert.equal(approvalVerdict.sideEffects.dispatch, false);
        assert.equal(approvalVerdict.sideEffects.runtimeExecutionRegistered, false);
        assert.equal(approvalVerdict.omitted.approvalTokenValues, true);
        assert.equal(approvalVerdict.omitted.approvalAuditPaths, true);
        assert.equal(approvalVerdict.omitted.approvalScopeHashes, true);
        assert.equal(approvalVerdict.omitted.workspaceRoot, true);
        assert.equal(approvalVerdict.omitted.cacheRoot, true);
        assert.equal(approvalVerdict.omitted.sha256, true);
...
test("RenderThumbnailTool execution rejects malformed renderer-service success responses", async () => {
    const originalFetch = globalThis.fetch;
    const calls: Array<{ url: unknown; init: any }> = [];
    globalThis.fetch = async (url: unknown, init?: any) => {
        calls.push({ url, init });
    ...
    try {
        const tool = new RenderThumbnailTool(
            mcpServerWithSse(async () => {
                throw new Error("render.thumbnail renderer-service execution must not call backend SSE");
            })
        );
