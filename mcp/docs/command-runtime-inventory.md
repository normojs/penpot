...
| `render.thumbnail` | `RenderThumbnailArgs`; MCP and CLI dry-run/client boundaries | dashboard thumbnail data/render/cache contract with `renderer-service` planning adapter, metadata-only availability probe, response/error normalization, and disabled client request scaffold; runtime execution remains unavailable | MCP/CLI dry-run plan with PNG thumbnail artifact metadata, renderer-service request shape, client config, availability status, response/error contracts, and `clientRequest.dispatch:false`; `dryRun:false`/execution reports `renderer_service_unavailable` and no runtime rendering | `command-runtime.test.mjs` consumes `render-thumbnail-contract-fixtures.json`, `render-thumbnail-runtime-boundary-fixtures.json`, and `render-thumbnail-renderer-service-fixtures.json`; MCP `ExportTools.test.ts` and CLI smoke tests cover dry-run, unavailable execution, client availability, response contract exposure, disabled client scaffold, and adapter errors |
...
- prototype/export: `prototype.create_overlay` and
  ...
  availability probes, P25.11 response/error normalization contracts, and
  P25.12 disabled client request scaffolding while runtime execution remains
  unavailable
