...
| `render.thumbnail` | Planned `RenderThumbnailArgs`; CLI dry-run/client boundary | dashboard thumbnail data/render/cache contract with `renderer-service` planning adapter; runtime execution and MCP tool remain unregistered | CLI dry-run/client plan with PNG thumbnail artifact metadata plus renderer-service request shape; no MCP tool registered and no runtime rendering | `command-runtime.test.mjs` consumes `render-thumbnail-contract-fixtures.json`, `render-thumbnail-runtime-boundary-fixtures.json`, and `render-thumbnail-renderer-service-fixtures.json`; `cli-smoke.test.mjs` covers dry-run and unavailable execution |
...
- prototype/export: `prototype.create_overlay` and
  ...
  P25.7 renderer-service API fixtures, and a P25.8 CLI dry-run/client boundary
