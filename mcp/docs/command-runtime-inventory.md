...
| `render.thumbnail` | `RenderThumbnailArgs`; MCP and CLI dry-run/client boundaries | dashboard thumbnail data/render/cache contract with `renderer-service` planning adapter; runtime execution remains unavailable | MCP/CLI dry-run plan with PNG thumbnail artifact metadata plus renderer-service request shape; `dryRun:false`/execution reports `renderer_service_unavailable` and no runtime rendering | `command-runtime.test.mjs` consumes `render-thumbnail-contract-fixtures.json`, `render-thumbnail-runtime-boundary-fixtures.json`, and `render-thumbnail-renderer-service-fixtures.json`; MCP `ExportTools.test.ts` and CLI smoke tests cover dry-run, unavailable execution, and adapter errors |
...
- prototype/export: `prototype.create_overlay` and
  ...
  and a P25.9 MCP planning-only dry-run tool while runtime execution remains
  unavailable
