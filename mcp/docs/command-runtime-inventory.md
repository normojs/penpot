...
| `render.thumbnail` | Planned `RenderThumbnailArgs` | descriptor-only dashboard thumbnail data/render/cache contract; future renderer-service boundary and API fixtures selected but unregistered | planned PNG thumbnail artifact/cache metadata plus future renderer-service request/response and resource-return shape; no MCP tool registered | `command-runtime.test.mjs` consumes `render-thumbnail-contract-fixtures.json`, `render-thumbnail-runtime-boundary-fixtures.json`, and `render-thumbnail-renderer-service-fixtures.json` |
...
- prototype/export: `prototype.create_overlay` and
  ...
  fixture-backed target/cache/artifact contract, a P25.6 renderer-service
  boundary audit, and P25.7 renderer-service API fixtures
