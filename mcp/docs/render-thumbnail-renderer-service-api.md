...
P26.54 adds optional controlled renderer-service process lifecycle for private checkout operators. Default `penpot-cli renderer-service start` remains no-spawn and returns the manual start command. Explicit `--spawn` or `PENPOT_RENDERER_SERVICE_LIFECYCLE=spawn|controlled|enabled` allows controlled spawn, health-watch, pid-file tracking, and `renderer-service stop`. Background daemons are not started without operator consent. The CLI/MCP execution gate is endpoint-first: a configured renderer-service endpoint (and normal caller-session auth) is enough to open health preflight and render dispatch for supported targets. Explicit `--render-thumbnail-execution renderer-service` remains supported for compatibility. Explicit disable values (`disabled`, `off`, `false`, `0`, `no`) keep execution closed. Unsafe auto-spawn, unreviewed materialization writes, and local-fs side effects remain gated; diagnostics stay redacted. With approved asset materialization, reviewed real-runtime module scaffold, reviewed installation, reviewed render dispatch, and backend source-data, `/thumbnail` can install the real runtime, call `renderThumbnail`, produce a non-empty PNG for one file or tagged-frame target, and persist through existing backend thumbnail RPCs. `/health` and `/thumbnail` report `bundledSceneBridgeRealScenePng`. Multi-target matrix expansion, broad default enablement, and credential/media/source value exposure remain gated. After ready preflight/dry-run and explicit `PENPOT_RENDERER_SERVICE_RUNTIME_ASSET_MATERIALIZATION_APPROVAL=approved` plus non-empty approval token and audit directory, `/health` and `/thumbnail` report `runtimeAssetMaterializationExecution` with status `executed` when public worker/render-wasm/rasterizer assets are copied into the cache root, hash-verified, and audited. Failures roll back written cache paths. Unapproved writes, browser startup before assets are ready, runtime value exposure, and default MCP/CLI enablement remain gated. After P26.46 installation and `PENPOT_RENDERER_SERVICE_BUNDLED_SCENE_BRIDGE_RENDER_DISPATCH_GATE=reviewed-dispatch`, `/thumbnail` may select the process-local registry runtime when no injected adapter is present, call `renderThumbnail` for refresh/miss paths, and map PNG bytes into the existing resource/persist pipeline. Injected `RendererServiceOptions.renderer` adapters still take precedence. `/health` and `/thumbnail` report `bundledSceneBridgeInstalledRuntimeRenderDispatch`. Default MCP/CLI enablement, unreviewed gates, asset materialization writes, and credential/media/source value exposure remain gated. `/health` and `/thumbnail` report `bundledSceneBridgeRealRuntimeModuleScaffold`, and the service-owned module `./bundled-scene-bridge-real-runtime.js` exports `createBundledSceneBridgeRealRendererRuntime`. The reviewed gate value is `reviewed-scaffold`; even when open the scaffold stays no-import/no-browser/no-dispatch and does not replace the stub factory. Asset loading, browser startup, render dispatch, materialization writes, and value exposure remain gated. `/health` and `/thumbnail` report `bundledSceneBridgeRealRuntimeValueContract` with status `planned-disabled`, documenting ownership for browser-backed render-wasm and frontend rasterizer paths, page handoff, render I/O, failure taxonomy, and a test matrix. The real runtime remains unimplemented and unselected; stub installation may remain occupied while browser startup, asset materialization writes, default MCP/CLI rendering, and value exposure stay gated. After P26.46 installation, `/health` and `/thumbnail` report `bundledSceneBridgeInstalledRuntimeLifecycle` with status `installed` when the process-local registry slot is occupied, close/rollback hooks are registered, and replacement remains unsupported. Diagnostics stay token-safe and no-dispatch: render dispatch, real scene assets, asset materialization writes, and runtime value exposure remain gated. After a planned P26.45 boundary, `/health` and `/thumbnail` report `bundledSceneBridgeRuntimeRegistryInstallation` with status `executed` when the service-owned stub runtime is created and installed into the process-local thumbnail runtime registry with close/rollback hooks. Installation remains no-dispatch: real scene rendering, browser startup beyond inert handles, asset materialization writes, backend/source-data reads beyond existing paths, local writes, runtime/option value exposure, and default MCP/CLI enablement stay gated.
