...
- P11.2 also adds a backend-safe layout subset for frame targets:
  `layout.type` can be `none`, `flex`, or the grid container track subset plus optional layout.cells placements.
...
| `token.get_mcp_status` | `get-current-mcp-token` | Current MCP `userToken`; returns token-safe presence/expiry only |
...
P25.78 render.thumbnail package materialization approval audit countersignature revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution enforcement policy result:
...
- Shared plans include
  ...
  policies, required enforcement inputs, enforcement checks, blocked enforcement
...
- MCP and CLI expose the verification revocation appeal resolution enforcement
  ...
  enforcement policies, identifying subjects or authorities, reading
  resolutions or resolution records, capturing enforcement reasons, computing
  scopes, selecting actions, storing or executing enforcement, enforcing
  resolutions, accepting or rejecting enforcement decisions, creating
  enforcement records, reading countersignatures or audit records, granting
...
P25.81 render.thumbnail package materialization approval audit countersignature revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution enforcement evidence attestation notarization policy result:
...
- MCP and CLI expose the verification revocation appeal resolution enforcement
  evidence attestation notarization policy plan in dry-run and unavailable
  execution payloads without selecting notarization policies, identifying
  notarization subjects or authorities, preparing, creating, validating,
  storing, publishing, or recording notarizations, reading, notarizing, or
  verifying attestations, reading evidence, enforcement, resolution, appeal,
  revocation, countersignature, or audit records, granting approval, creating
...
P25.82 render.thumbnail package materialization approval audit countersignature revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification policy result:
...
- Shared plans include
  ...
  enforcement evidence attestation notarization certification endorsement
  countersignature verification revocation appeal resolution enforcement
  evidence attestation notarization certification policies, required
  certification inputs, certification checks, blocked certification decisions,
  and no-op guarantees.
- MCP and CLI expose the verification revocation appeal resolution enforcement
  evidence attestation notarization certification policy plan in dry-run and
  unavailable execution payloads without selecting certification policies,
  identifying certification subjects or authorities, preparing, creating,
  validating, storing, publishing, or recording certifications, reading,
  certifying, or verifying notarizations, reading, notarizing, or verifying
  attestations, reading evidence, enforcement, resolution, appeal, revocation,
  countersignature, or audit records, granting approval, creating package
  directories, writing files, mutating workspace manifests, mutating lockfiles,
  running commands, emitting build output, starting processes, or registering
  runtime dispatch.
...
P25.83 render.thumbnail package materialization approval audit countersignature revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification endorsement policy result:
...
- Shared plans include
  ...
  decisions, and no-op guarantees.
- MCP and CLI expose the verification revocation appeal resolution enforcement
  evidence attestation notarization certification endorsement policy plan in
  dry-run and unavailable execution payloads without selecting endorsement
  policies, identifying endorsement subjects or authorities, preparing,
  creating, validating, storing, publishing, or recording endorsements,
  reading, endorsing, or verifying certifications, reading lower audit-chain
...
P25.84 render.thumbnail package materialization approval audit countersignature revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature policy result:
...
- Shared plans include
  ...
  countersignature verification revocation appeal resolution enforcement
  evidence attestation notarization certification endorsement countersignature
  policies, required endorsement-countersignature inputs, countersignature
  checks, blocked countersignature decisions, and no-op guarantees.
- MCP and CLI expose the verification revocation appeal resolution enforcement
  evidence attestation notarization certification endorsement countersignature
  policy plan in dry-run and unavailable execution payloads without selecting
  countersignature policies, identifying countersignature subjects or
  authorities, preparing, creating, validating, storing, publishing, or
  recording countersignatures, reading, countersigning, or verifying
  endorsements, reading audit records, granting approval, creating package
  directories, writing files, mutating workspace manifests, mutating lockfiles,
  running commands, emitting build output, starting processes, or registering
  runtime dispatch.
- `executableAdapterRegistrationScaffold.dispatch`, `networkDispatch`,
...
P25.85 render.thumbnail package materialization approval audit countersignature revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification policy result:
...
- Shared plans include
  ...
  defining metadata-only countersignature verification policies, required
  countersignature verification inputs, verification checks, blocked
  verification decisions, and no-op guarantees.
- MCP and CLI expose the countersignature verification plan in dry-run and
  unavailable execution payloads without selecting verification policies,
  identifying subjects or authorities, reading countersignatures or audit
  records, parsing payloads, verifying signatures, computing hashes, linking
  chains, storing verification results, granting approval, creating package
  directories, writing files, mutating workspace manifests, mutating lockfiles,
  running commands, emitting build output, starting processes, or registering
  runtime dispatch.

P25.86 render.thumbnail package materialization approval audit countersignature revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation policy result:
...
- Shared plans include
  ...
  defining metadata-only countersignature verification revocation policies,
  required revocation inputs, revocation checks, blocked revocation decisions,
  and no-op guarantees.
- MCP and CLI expose the countersignature verification revocation plan in
  ...
  computing scopes, preparing or storing requests, revoking verification
  records, creating revocation records, reading countersignatures or audit
  records, granting approval, creating package directories, writing files,
  mutating workspace manifests, mutating lockfiles, running commands, emitting
  build output, starting processes, or registering runtime dispatch.
- `executableAdapterRegistrationScaffold.dispatch`, `networkDispatch`,
...
P25.70 render.thumbnail package materialization approval audit countersignature
...
- Shared plans include
  ...
  defining metadata-only audit countersignature revocation appeal resolution
  enforcement evidence attestation notarization policies, required notarization
...
P25.71 render.thumbnail package materialization approval audit countersignature
revocation appeal resolution enforcement evidence attestation notarization
certification policy result:
...
- Shared plans include
  ...
  decisions, and no-op guarantees.
- MCP and CLI expose the audit countersignature revocation appeal resolution
  enforcement evidence attestation notarization certification policy plan in
  dry-run and unavailable execution payloads without selecting certification
  policies, identifying subjects or authorities, preparing, creating,
  validating, storing, publishing, or recording certifications, reading,
  certifying, or verifying notarizations, reading audit records,
  linking/verifying/signing/hashing certifications, granting approval,
  creating package directories, writing files, mutating workspace manifests,
  mutating lockfiles, running commands, emitting build output, starting
  processes, or registering runtime dispatch.
...
P25.73 render.thumbnail package materialization approval audit countersignature
revocation appeal resolution enforcement evidence attestation notarization
certification endorsement countersignature policy result:
...
- Shared plans include
  ...
  defining metadata-only audit countersignature revocation appeal resolution
  enforcement evidence attestation notarization certification endorsement
  countersignature policies, required countersignature inputs,
  countersignature checks, blocked countersignature decisions, and no-op
  guarantees.
- MCP and CLI expose the audit countersignature revocation appeal resolution
  enforcement evidence attestation notarization certification endorsement
  countersignature policy plan in dry-run and unavailable execution payloads
  without selecting countersignature policies, identifying subjects or
  authorities, preparing, creating, validating, storing, publishing, or
  recording countersignatures, reading, countersigning, or verifying
  endorsements, reading audit records, linking/verifying/signing/hashing
  countersignatures, granting approval, creating package directories, writing
  files, mutating workspace manifests, mutating lockfiles, running commands,
  emitting build output, starting processes, or registering runtime dispatch.
...
P25.74 render.thumbnail package materialization approval audit countersignature
revocation appeal resolution enforcement evidence attestation notarization
certification endorsement countersignature verification policy result:
...
- MCP and CLI expose the verification policy plan in dry-run and unavailable
  ...
  workspace manifests, mutating lockfiles, running commands, emitting build
  output, starting processes, or registering runtime dispatch.
- `countersignatureRead`, `countersignaturePayloadParsed`,
...
P25.75 render.thumbnail package materialization approval audit countersignature
revocation appeal resolution enforcement evidence attestation notarization
...
- Shared plans include
  ...
  defining metadata-only audit countersignature revocation appeal resolution
  enforcement evidence attestation notarization certification endorsement
  countersignature verification revocation policies, required revocation
  inputs, revocation checks, blocked revocation decisions, and no-op
  guarantees.
- MCP and CLI expose the verification revocation policy plan in dry-run and
  unavailable execution payloads without selecting revocation policies,
  identifying subjects or authorities, capturing reasons, computing scopes,
  storing or executing revocation requests, revoking verification records,
  creating revocation records, reading countersignatures or audit records,
  granting approval, creating package directories, writing files, mutating
  workspace manifests, mutating lockfiles, running commands, emitting build
  output, starting processes, or registering runtime dispatch.
...
P25.70 defines disabled package materialization approval audit countersignature
revocation appeal resolution enforcement evidence attestation notarization
policy.
...
P25.73 defines disabled package materialization approval audit countersignature
revocation appeal resolution enforcement evidence attestation notarization
certification endorsement countersignature policy.
P25.74 defines disabled package materialization approval audit countersignature
revocation appeal resolution enforcement evidence attestation notarization
certification endorsement countersignature verification policy.
...
P25.81 defines disabled package materialization approval audit countersignature
revocation appeal resolution enforcement evidence attestation notarization
certification endorsement countersignature verification revocation appeal
resolution enforcement evidence attestation notarization policy.
P25.82 defines disabled package materialization approval audit countersignature
revocation appeal resolution enforcement evidence attestation notarization
certification endorsement countersignature verification revocation appeal
resolution enforcement evidence attestation notarization certification policy.
P25.83 defines disabled package materialization approval audit countersignature
revocation appeal resolution enforcement evidence attestation notarization
certification endorsement countersignature verification revocation appeal
resolution enforcement evidence attestation notarization certification
endorsement policy.
P25.84 defines disabled package materialization approval audit countersignature
revocation appeal resolution enforcement evidence attestation notarization
certification endorsement countersignature verification revocation appeal
resolution enforcement evidence attestation notarization certification
...
P26.46 executes the guarded runtime registry installation.
P26.47 surfaces installed-runtime lifecycle diagnostics.
P26.48 plans the real bundled scene bridge runtime value contract.
P26.49 scaffolds the real bundled scene bridge runtime module.
P26.50 wires installed-runtime thumbnail render dispatch.
P26.51 executes approved runtime asset cache materialization.
P26.52 produces a minimal real-scene PNG through the installed bridge.
P26.53 reduces default `render.thumbnail` operator friction.
P26.54 adds optional controlled renderer-service process lifecycle. `penpot-cli renderer-service` keeps default no-spawn status/start planning, and only manages a process when operators opt in with `--spawn` or `PENPOT_RENDERER_SERVICE_LIFECYCLE=spawn`. Controlled mode records a host/port pid file, waits for `/health`, and supports `stop`. This remains operator-consented and does not auto-daemonize renderer-service for MCP/CLI by default. Command-runtime execution-gate planning is endpoint-first (`operatorFriction.endpointFirst`), so a configured renderer-service endpoint opens health preflight and dispatch without requiring a separate opt-in flag. Operators can still set `PENPOT_RENDER_THUMBNAIL_EXECUTION` / `--render-thumbnail-execution` to `disabled` (or `off`/`false`/`0`/`no`) to keep execution closed. Explicit `renderer-service` remains a recognized compatibility value. Auto-spawn, unreviewed materialization, and credential/media/source value exposure stay gated. The renderer-service health and thumbnail responses expose `bundledSceneBridgeRealScenePng`, and command-runtime normalizes the redacted summary into CLI/MCP `healthPreflight.bundledSceneBridgeRealScenePng`. After approved materialization and reviewed real-runtime/render-dispatch gates, the service can install the real runtime, render a non-empty PNG for one file or tagged-frame target, and persist through existing backend thumbnail RPCs while keeping multi-target matrix expansion and credential/media/source value exposure gated. The renderer-service health and thumbnail responses expose `runtimeAssetMaterializationExecution`, and command-runtime normalizes the redacted summary into CLI/MCP `healthPreflight.runtimeAssetMaterializationExecution`. Approved hosts copy reviewed public assets into the configured cache root with hash verification, audit metadata, and rollback on failure while keeping unapproved writes, browser startup before assets are ready, runtime value exposure, and default MCP/CLI enablement gated. The renderer-service health and thumbnail responses expose `bundledSceneBridgeInstalledRuntimeRenderDispatch`, and command-runtime normalizes the redacted summary into CLI/MCP `healthPreflight.bundledSceneBridgeInstalledRuntimeRenderDispatch`. With an installed registry runtime and `PENPOT_RENDERER_SERVICE_BUNDLED_SCENE_BRIDGE_RENDER_DISPATCH_GATE=reviewed-dispatch`, the service may call the installed `renderThumbnail` for file/frame refresh/miss paths and map PNG bytes into the existing resource/persist pipeline while preferring injected adapters and keeping default MCP/CLI enablement, unreviewed gates, asset materialization writes, and credential/media/source value exposure gated. The renderer-service health and thumbnail responses expose `bundledSceneBridgeRealRuntimeModuleScaffold`, and command-runtime normalizes the redacted summary into CLI/MCP `healthPreflight.bundledSceneBridgeRealRuntimeModuleScaffold`. The service-owned module `bundled-scene-bridge-real-runtime` defines `createBundledSceneBridgeRealRendererRuntime` behind `PENPOT_RENDERER_SERVICE_BUNDLED_SCENE_BRIDGE_REAL_RUNTIME_MODULE_GATE=reviewed-scaffold` while keeping default selection on the stub factory and refusing asset loading, browser startup, render dispatch, materialization writes, and value exposure. The renderer-service health and thumbnail responses expose `bundledSceneBridgeRealRuntimeValueContract`, and command-runtime normalizes the redacted summary into CLI/MCP `healthPreflight.bundledSceneBridgeRealRuntimeValueContract`. The planned contract replaces the stub-only runtime value plan with explicit browser-backed render-wasm/frontend rasterizer ownership, page handoff, render I/O, failure taxonomy, and test matrix metadata while keeping real runtime implementation, browser startup, asset materialization writes, default MCP/CLI rendering, and value exposure disabled. The renderer-service health and thumbnail responses expose `bundledSceneBridgeInstalledRuntimeLifecycle`, and command-runtime normalizes the redacted summary into CLI/MCP `healthPreflight.bundledSceneBridgeInstalledRuntimeLifecycle`. After P26.46 installation the diagnostics report slot occupancy, close/rollback readiness, and duplicate/replacement policy without enabling render dispatch or exposing runtime values. The renderer-service health and thumbnail responses expose `bundledSceneBridgeRuntimeRegistryInstallation`, and command-runtime normalizes the redacted summary into CLI/MCP `healthPreflight.bundledSceneBridgeRuntimeRegistryInstallation`. After a planned P26.45 boundary the service creates a stub runtime value, installs it into the process-local thumbnail runtime registry, and registers close/rollback hooks while keeping render dispatch, real scene assets, asset materialization writes, and value exposure disabled.
