...
| `render.thumbnail` | Name exists in `ToolNames.ts` and command-runtime descriptor-only entries; no registered MCP tool, CLI command, or executable adapter. P25.4 adds a fixture-backed contract, P25.6 audits the runtime boundary, and P25.7 defines renderer-service API fixtures. | Descriptor-only dashboard thumbnail contract; future dedicated thumbnail renderer service selected but unregistered. | File thumbnails use `get-file-data-for-thumbnail` plus `create-file-thumbnail`; tagged frame thumbnails use `fileId/pageId/objectId/tag` and `create-file-object-thumbnail`. P25.7 adds future service request/response fixtures for file refresh, file reuse, and tagged frame refresh. | Keep adapters empty until renderer-service implementation, dry-run/client boundary, cache probe, tagged-frame source data, resource normalization, auth, and tests exist. |
