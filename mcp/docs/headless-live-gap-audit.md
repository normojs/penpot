...
| `debug.get_plugin_state`, `debug.get_agent_logs` | Names exist in `ToolNames.ts`; command-runtime `DebugDiagnosticsCommandDescriptors` with empty adapters; not registered as MCP tools or CLI execution. | Descriptor-only planned (Phase 28 complete). | Diagnostics exist through status/log paths; map onto `mcp.get_status` / `mcp logs` projections. | `debug-diagnostics-descriptor-boundaries.md`; keep empty adapters until enablement/redaction gates exist; first executable candidate is `debug.get_plugin_state`. |
