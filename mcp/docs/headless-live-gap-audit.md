...
| `render.thumbnail` | Name exists in `ToolNames.ts` and command-runtime descriptor entries; no registered MCP tool or executable renderer. P25.4 adds a fixture-backed contract, P25.6 audits the runtime boundary, P25.7 defines renderer-service API fixtures, and P25.8 adds CLI dry-run/client planning. | Dashboard thumbnail contract with `renderer-service` planning adapter; runtime execution still unavailable. | File thumbnails use `get-file-data-for-thumbnail` plus `create-file-thumbnail`; tagged frame thumbnails use `fileId/pageId/objectId/tag` and `create-file-object-thumbnail`. CLI dry-run prints the future `thumbnail.render` service request. | Keep runtime execution disabled until renderer-service implementation, cache probe, tagged-frame source data, resource normalization, auth, and tests exist. |
