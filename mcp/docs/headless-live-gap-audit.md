...
| `debug.get_plugin_state`, `debug.get_agent_logs` | Names exist in `ToolNames.ts`, not registered. | Descriptor-only planned (Phase 28). | Diagnostics exist through status/log paths; map onto `mcp.get_status` / `mcp logs` projections. | P28.1 planning doc: `debug-diagnostics-descriptor-boundaries.md`; keep empty adapters until enablement/redaction gates exist. |
