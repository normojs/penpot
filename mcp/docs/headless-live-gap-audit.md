...
P25.71 defines the package materialization approval audit countersignature revocation appeal resolution enforcement evidence attestation notarization certification policy. | Dashboard thumbnail contract with `renderer-service` planning adapter; runtime execution still unavailable. | Dry-run and unavailable execution payloads expose countersignature revocation appeal resolution enforcement evidence attestation notarization certification policy metadata without certifying notarizations, reading notarizations or audit records, storing certification records, writing files, starting processes, or registering dispatch. | Keep runtime execution disabled until opt-in config surfaces, renderer-service implementation, workspace wiring, health preflight, cache probe, tagged-frame source data, resource normalization, auth, integration tests, and runtime registration exist. |
P25.72 defines the package materialization approval audit countersignature revocation appeal resolution enforcement evidence attestation notarization certification endorsement policy. | Dashboard thumbnail contract with `renderer-service` planning adapter; runtime execution still unavailable. | Dry-run and unavailable execution payloads expose countersignature revocation appeal resolution enforcement evidence attestation notarization certification endorsement policy metadata without endorsing certifications, reading certifications or audit records, storing endorsement records, writing files, starting processes, or registering dispatch. | Keep runtime execution disabled until opt-in config surfaces, renderer-service implementation, workspace wiring, health preflight, cache probe, tagged-frame source data, resource normalization, auth, integration tests, and runtime registration exist. |
...
| `debug.get_plugin_state` | Registered MCP tool (gated) + command-runtime `local` adapter + CLI `debug plugin-state`. | Backend/local projection of existing status surfaces. | `DebugGetPluginStateTool` projects plugin counts/session/fileContext from `getStatus()`; requires `PENPOT_MCP_ENABLE_DEBUG_TOOLS=true`. | Prefer `mcp.get_status` for normal diagnostics. |
| `debug.get_agent_logs` | Command-runtime descriptor only (`adapters: []`); not registered as MCP/CLI execution. | Descriptor-only planned. | Map onto `mcp logs` metadata when redaction fixtures exist. | Operators: `penpot-cli mcp logs`. |
...
P25.80 extends the `render.thumbnail` renderer-service planning payload with
...
countersignature revocation appeal resolution enforcement evidence attestation
notarization certification endorsement countersignature verification revocation
appeal resolution enforcement evidence attestation metadata while keeping
attestation policy selection, subject/authority identification, attestation
creation/storage/publication, attestation bundle storage, evidence record
reads/attestation/verification, evidence bundle reads, enforcement, resolution,
appeal, revocation, countersignature, and audit record reads, materialization
approval, files written, command execution, build output, process startup, and
runtime registration disabled.
...
notarization creation/storage/publication, notarization record storage/
publication, attestation reads/notarization/verification, evidence,
enforcement, resolution, appeal, revocation, countersignature, and audit record
reads, materialization approval, files written, command execution, build
output, process startup, and runtime registration disabled.
...
identification, certification creation/storage/publication, certification
record storage/publication, notarization reads/certification/verification,
attestation reads/notarization/verification, evidence, enforcement,
...
subject/authority identification, endorsement creation/storage/publication,
endorsement record storage/publication, certification reads/endorsement/
verification, lower audit-chain reads, materialization approval, files written,
command execution, build output, process startup, and runtime registration
disabled.
...
metadata-only package materialization approval audit countersignature revocation
appeal resolution enforcement evidence attestation notarization certification
policy. It defines future countersignature revocation appeal resolution
enforcement evidence attestation notarization certification policies, required
certification inputs, certification checks, and blocked certification decision
metadata while keeping certification policy selection, subject/authority
identification, notarization and audit record reads, notarization
certification/verification, certification creation/storage/publication,
certification linking/verification/signing/hashing, approval, file writes,
workspace mutation, command execution, build output, package creation, process
startup, and runtime registration disabled.
...
metadata-only package materialization approval audit countersignature revocation
appeal resolution enforcement evidence attestation notarization certification
endorsement policy. It defines future countersignature revocation appeal
resolution enforcement evidence attestation notarization certification
endorsement policies, required endorsement inputs, endorsement checks, and
blocked endorsement decision metadata while keeping endorsement policy
selection, subject/authority identification, certification and audit record
reads, certification endorsement/verification, endorsement creation/storage/
publication, endorsement linking/verification/signing/hashing, approval, file
writes, workspace mutation, command execution, build output, package creation,
process startup, and runtime registration disabled.
...
verification revocation, revocation record storage/publication, countersignature
