...
- P26.53 reduces default `render.thumbnail` operator friction with endpoint-first execution: a configured renderer-service endpoint is enough to open the gate, while `disabled`/`off`/`false`/`0`/`no` still closes it and diagnostics remain redacted.
- P26.52 produces a minimal real-scene PNG through the installed real bundled scene bridge runtime (`bundledSceneBridgeRealScenePng`) for one file/tagged-frame target after approved materialization, reviewed real-runtime scaffold, and reviewed render dispatch, persisting through the existing backend thumbnail path while keeping multi-target expansion and value exposure gated.
- P26.51 executes approved runtime asset cache materialization (`runtimeAssetMaterializationExecution`) with hash verification, audit metadata, and rollback on failure after preflight/dry-run readiness and `PENPOT_RENDERER_SERVICE_RUNTIME_ASSET_MATERIALIZATION_APPROVAL=approved`.
- P26.50 wires installed registry runtimes into gated thumbnail render dispatch (`bundledSceneBridgeInstalledRuntimeRenderDispatch`) behind `PENPOT_RENDERER_SERVICE_BUNDLED_SCENE_BRIDGE_RENDER_DISPATCH_GATE=reviewed-dispatch`, preferring injected adapters and mapping PNG bytes through the existing resource/persist pipeline.
- P26.49 scaffolds the gated real bundled scene bridge runtime module (`bundledSceneBridgeRealRuntimeModuleScaffold` + `bundled-scene-bridge-real-runtime`) behind `PENPOT_RENDERER_SERVICE_BUNDLED_SCENE_BRIDGE_REAL_RUNTIME_MODULE_GATE=reviewed-scaffold` without default asset loading, browser startup, or factory selection over the stub.
- P26.48 plans the real bundled scene bridge runtime value contract (`bundledSceneBridgeRealRuntimeValueContract`) for browser-backed render-wasm/frontend rasterizer ownership, page handoff, render I/O, and failure taxonomy while keeping the real runtime unimplemented.
- P26.47 surfaces token-safe installed-runtime lifecycle and rollback diagnostics (`bundledSceneBridgeInstalledRuntimeLifecycle`) after P26.46 installation without enabling render dispatch or value exposure.
- P26.46 executes the guarded bundled scene bridge runtime registry installation (`bundledSceneBridgeRuntimeRegistryInstallation`) after a planned P26.45 boundary, installing a process-local stub runtime with close/rollback hooks while keeping render dispatch and value exposure disabled.
...
- Add a no-dispatch bundled scene bridge runtime registry registration boundary through renderer-service, command-runtime, CLI, and fixtures, documenting registry slot ownership, duplicate/replacement policy, lifecycle cleanup, and runtime availability while keeping runtime installation, registration, dispatch, browser startup, asset loading, reads, writes, and value exposure disabled.
- Execute the guarded bundled scene bridge runtime registration preflight with explicit `PENPOT_RENDERER_SERVICE_BUNDLED_SCENE_BRIDGE_RUNTIME=registration-preflight` opt-in, reporting ready/blocked outcomes through renderer-service, command-runtime, CLI, and MCP health summaries while keeping runtime registration, render dispatch, browser startup, asset loading, reads, writes, and value exposure disabled.
...
- Persist renderer-service rendered file-thumbnail PNGs through backend `create-file-thumbnail` after runtime adapter success, returning backend resource metadata with token-safe `persistOutput` execution summaries while keeping tagged-frame persistence and value exposure disabled.
- Execute renderer-service tagged-frame refresh source-data reads through backend `get-file-frame-data-for-thumbnail` and pass frame targets to runtime adapters, returning in-memory PNG resources while keeping tagged-frame persistence and value exposure disabled.
...
- Add the P26.21 bundled runtime asset materialization preflight scaffold to renderer-service health and thumbnail responses, listing read-only existence/hash/cache-output readiness checks and failure taxonomy while keeping file reads, hash computation, browser lifecycle, and runtime registration disabled.
- Execute the P26.22 bundled runtime asset preflight read-only slice for explicitly configured renderer-service hosts, reporting ready/degraded asset/cache/hash readiness while keeping browser startup, runtime adapter import, runtime asset loading, asset materialization, local writes, network dispatch, runtime registration, and value exposure disabled.
- Surface P26.23 runtime asset preflight readiness through CLI/MCP renderer-service health preflight diagnostics, reporting ready/degraded counts and ids while omitting roots, paths, hashes, token values, source data, page data, artifact data, and media values.
- Add renderer-service runtime asset preflight operator configuration via explicit read-only environment inputs, with `penpot-cli renderer-service status/start` reporting the manual host configuration, default cache root, diagnostics surface, and no-effect lifecycle plan.
- Normalize runtime asset preflight degraded and invalid diagnostics with stable codes plus redacted next-action guidance for missing public assets, missing cache assets, hash failures, unavailable cache outputs, and invalid operator configuration.
- Add metadata-only runtime asset materialization dry-run and approval planning to renderer-service health/thumbnail responses plus CLI/MCP summaries, reporting copy/cache plan readiness and a closed approval gate while keeping asset copying, browser startup, runtime loading, local writes, runtime registration, and value exposure disabled.
- Add a disabled runtime asset materialization approval token/config/audit scaffold to renderer-service health/thumbnail responses plus CLI/MCP summaries, exposing future approval env names and audit metadata while keeping token reads, token acceptance, audit writes, cache writes, runtime loading, and value exposure disabled.
- Report unsupported runtime asset materialization approval configuration attempts with redacted P26.28 diagnostics across renderer-service, command-runtime, `penpot-cli`, and MCP, exposing only configured booleans and next actions while keeping token values unread, audit paths omitted, approvals unaccepted, and writes disabled.
- Add a P26.29 metadata-only runtime asset materialization approval readiness verdict to renderer-service health/thumbnail responses plus CLI/MCP summaries, computing blocked checks from dry-run and approval configuration diagnostics while keeping approvals untrusted, token reads disabled, audit writes disabled, materialization writes disabled, and dispatch disabled.
- Add an explicitly opted-in P26.30 browser-backed renderer-service fixture runtime via `PENPOT_RENDERER_SERVICE_BROWSER_FIXTURE_RUNTIME=enabled`, with Playwright canvas PNG output, runtime adapter cleanup hooks, lifecycle coverage, and no claim of real Penpot scene rendering.
- Surface redacted P26.31 browser fixture runtime lifecycle diagnostics through renderer-service health/thumbnail responses, `penpot-cli` lifecycle status, and MCP/CLI `render.thumbnail` health preflight summaries without exposing paths, source/page data, artifact/media bytes, or tokens.
- Add the P26.32 bundled scene bridge adapter contract to renderer-service health/thumbnail responses plus MCP/CLI health preflight summaries, defining the future adapter module, asset prerequisites, browser page handoff, and render input/output rules while keeping runtime import, registration, browser startup, asset materialization, writes, and value exposure disabled.
...
- Add metadata-only renderer-service P25.140 evidence attestation notarization certification endorsement countersignature verification policy planning for future `render.thumbnail` package materialization approval audit execution across command-runtime, CLI, MCP, and fixtures while keeping countersignature verification, countersignature/endorsement/certification/notarization/attestation/evidence/audit record reads, materialization, and dispatch disabled.
...
- Add metadata-only renderer-service P25.135 evidence attestation policy planning for future `render.thumbnail` package materialization approval audit execution across command-runtime, CLI, MCP, and fixtures while keeping attestation creation, evidence/audit record reads, materialization, and dispatch disabled.
- Add metadata-only renderer-service countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution enforcement evidence policy planning for `render.thumbnail` across command-runtime, CLI, MCP, and fixtures while keeping evidence collection, enforcement/evidence/audit record reads, materialization, and dispatch disabled.
- Add metadata-only renderer-service countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution enforcement policy planning for `render.thumbnail` across command-runtime, CLI, MCP, and fixtures while keeping countersignature verification revocation appeal resolution enforcement, enforcement/resolution/audit record reads, materialization, and dispatch disabled.
- Add metadata-only renderer-service countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution policy planning for `render.thumbnail` across command-runtime, CLI, MCP, and fixtures while keeping countersignature verification revocation appeal resolution, resolution/audit record reads, materialization, and dispatch disabled.
- Add metadata-only renderer-service countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal policy planning for `render.thumbnail` across command-runtime, CLI, MCP, and fixtures while keeping countersignature verification revocation appeals, appeal/audit record reads, materialization, and dispatch disabled.
- Add metadata-only renderer-service countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation policy planning for `render.thumbnail` across command-runtime, CLI, MCP, and fixtures while keeping countersignature verification revocation, verification/audit record reads, materialization, and dispatch disabled.
- Add metadata-only renderer-service countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification policy planning for `render.thumbnail` across command-runtime, CLI, MCP, and fixtures while keeping countersignature verification, countersignature/audit record reads, materialization, and dispatch disabled.
- Add metadata-only renderer-service countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature policy planning for `render.thumbnail` across command-runtime, CLI, MCP, and fixtures while keeping endorsement countersignature, record reads, materialization, and dispatch disabled.
- Add metadata-only renderer-service countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification endorsement policy planning for `render.thumbnail` across command-runtime, CLI, MCP, and fixtures while keeping endorsement, record reads, materialization, and dispatch disabled.
- Add metadata-only renderer-service countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification policy planning for `render.thumbnail` across command-runtime, CLI, MCP, and fixtures while keeping notarization certification, record reads, materialization, and dispatch disabled.
- Add metadata-only renderer-service countersignature verification revocation appeal resolution enforcement evidence attestation notarization policy planning for `render.thumbnail` across command-runtime, CLI, MCP, and fixtures while keeping attestation notarization, record reads, materialization, and dispatch disabled.
- Add metadata-only renderer-service countersignature verification revocation appeal resolution enforcement evidence attestation policy planning for `render.thumbnail` across command-runtime, CLI, MCP, and fixtures while keeping evidence attestation, record reads, materialization, and dispatch disabled.
- Add metadata-only renderer-service package materialization approval audit countersignature verification revocation appeal resolution enforcement evidence policy planning for future `render.thumbnail` execution.
- Add metadata-only renderer-service certification endorsement countersignature verification revocation appeal resolution policy planning for `render.thumbnail` across command-runtime, CLI, MCP, and fixtures while keeping appeal resolution execution, record reads, materialization, and dispatch disabled.
- Add metadata-only renderer-service certification endorsement countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal policy planning for `render.thumbnail` across command-runtime, CLI, MCP, and fixtures while keeping appeal execution, appeal decision handling, record reads, materialization, and dispatch disabled.
- Add metadata-only renderer-service certification endorsement countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation policy planning for `render.thumbnail` across command-runtime, CLI, MCP, and fixtures while keeping countersignature verification revocation, verification/audit record reads, materialization, and dispatch disabled.
- Add metadata-only renderer-service certification endorsement countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification policy planning for `render.thumbnail` across command-runtime, CLI, MCP, and fixtures while keeping countersignature verification, countersignature/audit record reads, materialization, and dispatch disabled.
- Add metadata-only renderer-service certification endorsement countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature policy planning for `render.thumbnail` across command-runtime, CLI, MCP, and fixtures while keeping endorsement countersignature, record reads, materialization, and dispatch disabled.
- Add metadata-only renderer-service certification endorsement countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification endorsement policy planning for `render.thumbnail` across command-runtime, CLI, MCP, and fixtures while keeping certification endorsement, record reads, materialization, and dispatch disabled.
- Add metadata-only renderer-service certification endorsement countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification policy planning for `render.thumbnail` across command-runtime, CLI, MCP, and fixtures while keeping notarization certification, record reads, materialization, and dispatch disabled.
- Add metadata-only renderer-service certification endorsement countersignature verification revocation appeal resolution enforcement evidence attestation notarization policy planning for `render.thumbnail` across command-runtime, CLI, MCP, and fixtures while keeping attestation notarization, record reads, materialization, and dispatch disabled.
- Add metadata-only renderer-service certification endorsement countersignature verification revocation appeal resolution enforcement evidence attestation policy planning for `render.thumbnail` across command-runtime, CLI, MCP, and fixtures while keeping evidence attestation, record reads, materialization, and dispatch disabled.
...
- Add metadata-only renderer-service countersignature verification revocation appeal resolution enforcement policy planning for `render.thumbnail` across command-runtime, CLI, MCP, and fixtures while keeping resolution enforcement, record reads, materialization, and dispatch disabled.
...
- Add workspace MCP menu controls to manually bind or unbind the current file as the active MCP file context, reflecting binding, releasing, and bound states.
- Add MCP file-context lifecycle tests for frontend state transitions and server registry bind/release/reconnect behavior.
- Add typed MCP page tools for listing, creating, renaming, and switching pages in the bound file context.
- Add typed MCP shape creation tools for frames, rectangles, text, and image-backed rectangles in the bound file context.
- Add typed MCP shape update and delete tools for geometry, style, text, and basic board layout edits in the bound file context.
- Add typed MCP prototype tools for creating flows and navigate-to interactions in the bound file context.
- Add typed MCP export and preview tools that return shape, page, and selection renders as base64 data in the bound file context.
...
- Add a shared `@penpot/command-runtime` adapter-selection helper and report adapter-selection metadata from `penpot-cli` page/export commands and MCP page tools.
- Wire backend-command shape creation into MCP `shape.create_frame` / `shape.create_rect` / `shape.create_text` and add matching `penpot-cli shape create-*` commands.
- Add backend/common `update-file-shape` and `delete-file-shape` headless commands for simple frame, rectangle, and text shape revisions without a live browser workspace.
...
- Add MCP server/plugin version and capability negotiation with token-safe status diagnostics.
- Add MCP diagnostics in Integrations settings, including connection, compatibility, file context, logs, and last-error state.
- Add configurable MCP destructive-action confirmations for `shape.delete`, with remote/multi-user defaults and status diagnostics.
...
- Add shared command error codes and adapter-selection reason helpers for MCP and CLI command runtime paths.
...
- Add renderer-service package materialization approval audit provenance policy planning for future `render.thumbnail` execution.
- Add renderer-service package materialization approval audit custody policy planning for future `render.thumbnail` execution.
...
- Add renderer-service package materialization approval audit endorsement policy planning for future `render.thumbnail` execution.
- Add renderer-service package materialization approval audit countersignature policy planning for future `render.thumbnail` execution.
- Add renderer-service package materialization approval audit countersignature verification policy planning for future `render.thumbnail` execution.
- Add renderer-service package materialization approval audit countersignature revocation policy planning for future `render.thumbnail` execution.
...
- Add renderer-service package materialization approval audit countersignature revocation appeal resolution enforcement evidence attestation notarization certification policy planning for future `render.thumbnail` execution.
- Add renderer-service package materialization approval audit countersignature revocation appeal resolution enforcement evidence attestation notarization certification endorsement policy planning for future `render.thumbnail` execution.
- Add renderer-service package materialization approval audit countersignature revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature policy planning for future `render.thumbnail` execution.
- Add renderer-service package materialization approval audit countersignature revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification policy planning for future `render.thumbnail` execution.
- Add renderer-service package materialization approval audit countersignature revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation metadata-only policy planning for future `render.thumbnail` execution.
- Add renderer-service package materialization approval audit countersignature revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal policy planning for future `render.thumbnail` execution.
- Add renderer-service package materialization approval audit countersignature revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution policy planning for future `render.thumbnail` execution.
- Add renderer-service package materialization approval audit countersignature revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution policy planning for future `render.thumbnail` execution.
- Add renderer-service package materialization approval audit countersignature revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution enforcement policy planning for future `render.thumbnail` execution.
- Add renderer-service package materialization approval audit countersignature revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution enforcement policy test and fixture coverage for future `render.thumbnail` execution.
- Add renderer-service package materialization approval audit countersignature revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution enforcement evidence policy planning for future `render.thumbnail` execution.
- Add renderer-service package materialization approval audit countersignature revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution enforcement evidence attestation policy planning for future `render.thumbnail` execution.
- Add renderer-service package materialization approval audit countersignature revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution enforcement evidence attestation notarization policy planning for future `render.thumbnail` execution.
- Add renderer-service package materialization approval audit countersignature revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification policy planning for future `render.thumbnail` execution.
- Add renderer-service package materialization approval audit countersignature revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification endorsement policy planning for future `render.thumbnail` execution.
- Add renderer-service package materialization approval audit countersignature revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution enforcement evidence policy planning for future `render.thumbnail` execution.
- Add renderer-service package materialization approval audit countersignature revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution enforcement evidence attestation policy planning for future `render.thumbnail` execution.
- Add renderer-service package materialization approval audit countersignature revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution enforcement evidence attestation notarization policy planning for future `render.thumbnail` execution.
- Add renderer-service package materialization approval audit countersignature revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification policy planning for future `render.thumbnail` execution.
...
- Add renderer-service package materialization approval audit countersignature revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification endorsement policy planning for future `render.thumbnail` execution.
- Add renderer-service package materialization approval audit countersignature revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature policy planning for future `render.thumbnail` execution.
- Add renderer-service package materialization approval audit countersignature revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification metadata-only policy planning and fixture coverage for future `render.thumbnail` execution.
- Add renderer-service package materialization approval audit countersignature revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature policy planning for future `render.thumbnail` execution.
- Add renderer-service package materialization approval audit countersignature revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature metadata-only policy planning and fixture coverage for future `render.thumbnail` execution.
- Add renderer-service package materialization approval audit countersignature revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification metadata-only policy planning and fixture coverage for future `render.thumbnail` execution.
- Add renderer-service package materialization approval audit countersignature revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation metadata-only policy planning and fixture coverage for future `render.thumbnail` execution.
- Add renderer-service package materialization approval audit countersignature revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal resolution enforcement evidence attestation notarization certification endorsement countersignature verification revocation appeal metadata-only policy planning and fixture coverage for future `render.thumbnail` execution.
...
- Register MCP `shape.set_layout` and `shape.set_style` aliases that forward to the existing `shape.update` backend-command and plugin-live paths.
- Add `penpot-cli shape set-layout` and `shape set-style` CLI aliases over `shape.update` with layout/style-scoped validation.
- Document the prototype interaction stable identity audit and fixtures, selecting future persisted interaction UUIDs while preserving source-shape/index compatibility.
- Add prototype interaction identity metadata to read summaries, exposing optional `interactionId` plus explicit stable-id or source-index fallback metadata through common/backend, MCP, and `penpot-cli`.
...
- Add a file-data migration that assigns stable ids to legacy prototype interactions missing ids and repairs duplicate interaction ids while preserving existing unique ids, order, and payload fields.
...
- Add a closed renderer-service execution gate and integration-test plan for future `render.thumbnail` dispatch.
- Add a disabled renderer-service health preflight and executable client harness plan for future `render.thumbnail` dispatch.
- Add a disabled renderer-service dispatch adapter boundary for future `render.thumbnail` execution.
- Add renderer-service opt-in configuration surface planning for future `render.thumbnail` execution.
- Add renderer-service unavailable error taxonomy planning for future `render.thumbnail` execution.
- Add renderer-service integration fixture harness planning for future `render.thumbnail` execution.
- Add renderer-service dispatch registration preflight planning for future `render.thumbnail` execution.
- Add renderer-service executable adapter registration scaffold planning for future `render.thumbnail` execution.
- Add renderer-service adapter registry manifest planning for future `render.thumbnail` execution.
- Add renderer-service enablement checklist planning for future `render.thumbnail` execution.
- Add renderer-service implementation slice audit planning for future `render.thumbnail` execution.
- Add renderer-service health/no-op contract fixture planning for future `render.thumbnail` execution.
- Add renderer-service no-op service host scaffold planning for future `render.thumbnail` execution.
- Add renderer-service host lifecycle test fixture planning for future `render.thumbnail` execution.
...
- Add renderer-service package build verification planning for future `render.thumbnail` execution.
- Add renderer-service package materialization checklist planning for future `render.thumbnail` execution.
- Add renderer-service package creation dry-run summary planning for future `render.thumbnail` execution.
- Add renderer-service package creation file manifest planning for future `render.thumbnail` execution.
- Add renderer-service package materialization approval gate planning for future `render.thumbnail` execution.
- Add renderer-service package materialization execution dry-run planning for future `render.thumbnail` execution.
- Add renderer-service package materialization write contract planning for future `render.thumbnail` execution.
- Add renderer-service package materialization rollback contract planning for future `render.thumbnail` execution.
- Add renderer-service package materialization verification manifest planning for future `render.thumbnail` execution.
- Add renderer-service package materialization final approval checklist planning for future `render.thumbnail` execution.
- Add renderer-service package materialization explicit approval token planning for future `render.thumbnail` execution.
- Add renderer-service package materialization approval audit trail planning for future `render.thumbnail` execution.
- Add renderer-service package materialization approval replay guard planning for future `render.thumbnail` execution.
- Add renderer-service package materialization approval expiry policy planning for future `render.thumbnail` execution.
- Add renderer-service package materialization approval revocation policy planning for future `render.thumbnail` execution.
- Add renderer-service package materialization approval scope binding policy planning for future `render.thumbnail` execution.
- Add renderer-service package materialization approval operator confirmation policy planning for future `render.thumbnail` execution.
- Add renderer-service package materialization approval emergency stop policy planning for future `render.thumbnail` execution.
- Add renderer-service package materialization approval readiness verdict policy planning for future `render.thumbnail` execution.
- Add renderer-service package materialization approval execution handoff policy planning for future `render.thumbnail` execution.
- Add renderer-service package materialization approval post-handoff audit policy planning for future `render.thumbnail` execution.
- Add renderer-service package materialization approval audit countersignature verification revocation appeal policy planning for future `render.thumbnail` execution.
